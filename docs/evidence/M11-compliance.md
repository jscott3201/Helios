...
| M11-T04 | Add request admission and memory guard errors. | `admit_chat_request`; tests `admission_and_memory_guard_return_stable_error_codes`; smoke report context/memory guard gates. | Complete | Token counting is deterministic estimate for stub backend. |
...
- Compliant: localhost OpenAI-compatible chat completions subset, streaming SSE, model/adapters/health/metrics endpoints, stable JSON errors, request and memory guards, read-only/stub TUI control endpoints, TUI live HTTP provider, remote adapter loading disabled, and deterministic evidence.
