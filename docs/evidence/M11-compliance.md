...
explicit. Admission memory is backend-aware: Stub keeps the lightweight
stub-era estimate with no native resident-weights charge, while RealHelper and
PersistentNative use measured XR51/P04 constants plus the conservative
`max(ceil(words * 13 / 10), ceil(prompt_bytes / 2.25))` prompt-token bound.
...
| M11-T04 | Add request admission and memory guard errors. | `admit_chat_request`; tests `admission_and_memory_guard_return_stable_error_codes`, `admission_estimator_matches_f8_memory_table`, `stub_admission_does_not_charge_native_weights_floor`, `native_admission_charges_weights_floor`, `admission_token_estimate_covers_real_workload_corpus`, `native_admission_rejects_real_16k_workload_at_tiny16_budget`, `admission_guard_rejects_16k_unchunked_and_admits_chunked`; smoke report context/memory guard gates. | Complete | XR53 preserves stub admission behavior and applies measured real-server memory constants only to model-backed backends. |
