...
Decision: `accept_candidate`, env-gated.
...
- Enabled real-margin records expose `trace_top_k=5` and pass trace
  spot-checks.
...
python3 scripts/xr57_trace_spotcheck.py --records benchmarks/out/XR57-real-margins/candidate-real-margins-enabled-final-n2n4/records.jsonl --min-events 3 --out benchmarks/out/XR57-real-margins/xr57-trace-spotcheck-final.json
...
- Trace validation:
  ...
  for 3 recorded events.
- Drafter score parity:
  `benchmarks/out/XR57-real-margins/drafter-score-parity-final/` completed.
  Native draft tokens `[236792,236865,22592,236779]` matched PyTorch pinned
  tokens; pinned native/PyTorch drafter logits and margins matched within
  `0.5` bf16 tolerance. The stale XR54-R two-token reference mismatch is a
...
fields. Use only `GEMMA4D_EXPERIMENTAL_MTP_REAL_MARGINS=1` ABI-v4 records for
real adaptive-N signal analysis. XR57 delivers signals only; adaptive-N policy
logic remains wave-5.
