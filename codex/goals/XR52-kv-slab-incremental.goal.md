...
The code branch retains the safe full-attention slab append, logical-only clone
and snapshot serialization, verifier timing split, and dead verifier cleanup for
review, but it is not ready to ship as a default-on optimization. XR53 was not
started because the handoff requires it to be based on an accepted XR52 decode
baseline.
