...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let blockers = startup_blockers(&args, &draft_config);
    let run_id = run_id();
    ...
    let summary = Summary {
        ...
        measurement_notes: vec![
            "This XR60 harness slice is fail-closed scaffolding. It emits the required artifact shape and blocks until native DSpark draft execution is implemented.",
...
fn startup_blockers(args: &Args, draft_config: &DsparkConfigInspection) -> Vec<String> {
    ...
    blockers.push("native DSpark draft execution is not implemented yet".to_owned());
    blockers
}
...
fn render_report(summary: &Summary) -> String {
    ...
    out.push_str(
        "This artifact was generated by the XR60 fixed-prefix harness scaffold. Native DSpark draft execution is not implemented in this slice, so all records are blocked and no speed claim is made.\n\n",
    );
