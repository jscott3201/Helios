...
typedef struct Gemma4StepResult {
    ...
    void* native_last_hidden; /* opaque native-owned view; do not dereference in Rust */
} Gemma4StepResult;
...
/* M06+ */
Gemma4Status gemma4_load_drafter(const Gemma4LoadConfig* config, Gemma4Target* target, Gemma4Drafter** out);
Gemma4Status gemma4_free_drafter(Gemma4Drafter* drafter);
Gemma4Status gemma4_mtp_draft_block(Gemma4Drafter* drafter, Gemma4KvCache* cache, uint32_t block_size, int32_t* out_tokens, size_t* inout_count);
Gemma4Status gemma4_verify_tokens(Gemma4Target* target, Gemma4KvCache* cache, const int32_t* draft_tokens, size_t draft_count, Gemma4StepResult* out);

/* M10+ */
Gemma4Status gemma4_adapter_load(Gemma4Target* target, const char* manifest_json, Gemma4Adapter** out);
Gemma4Status gemma4_adapter_free(Gemma4Adapter* adapter);
Gemma4Status gemma4_adapter_activate(Gemma4Target* target, Gemma4KvCache* cache, const char* adapter_id);
Gemma4Status gemma4_adapter_deactivate(Gemma4Target* target, Gemma4KvCache* cache);
