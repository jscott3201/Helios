...
| 2026-07-03 | XR54 MTP drafter position pin | Needs more data | local XR54 position-pin changes | `native_mtp_position_pin_xr48_config` | `benchmarks/out/XR54-mtp-position-pin/{rung10-native-mtp,pinned-xr48-config}` plus `codex/goals/XR54-mtp-position-pin.goal.md` | Changed `NativeMtpAssistantModel::draft_block` to pass constant `first_position` to every assistant step, matching Hugging Face `SinglePositionMultiTokenCandidateGenerator` constant `position_ids` behavior. Verification passed: `cargo fmt --all --check`; `git diff --check`; `cargo test -p gemma4d-ffi --lib`; `cargo test -p gemma4d-server --all-targets`; `cargo test -p gemma4d-bench --example xr15_mtp_policy_variance_ab --no-run`; native P05 smoke with block sizes 1 and 2. The XR48-config holdout used `GEMMA4D_EXPERIMENTAL_MTP_BLOCK_PREFIX_ROLLBACK=1`, `GEMMA4D_EXPERIMENTAL_MTP_LAZY_SECOND_DRAFT=1`, `--adaptive-zero-accept-run 3`, `--adaptive-min-generated-tokens 12`, `3` measured trials plus `1` warmup, `32` generated tokens, and the three 1K workloads. It completed with `12/12` exact records and `9/9` measured exact records, but the acceptance prediction was refuted: all measured draft-token arrays were byte-identical to XR48. Per-slot acceptance remained `chat_short_1k_001` slot0 `36/60`, slot1 `33/36`; `tool_json_1k_001` slot0 `39/57`, slot1 `36/39`; `mtp_candidate_1k_001` slot0 `18/27`, slot1 `3/18`. Net-latency guarded policy still selected only chat/tool with aggregate `14.598%`; `mtp_candidate_1k_001` stayed rejected (`2883.842 -> 2987.766 ms`, `-3.604%`, auto-disabled `3/3`). XR55 must not start until the drafter-only PyTorch parity contingency explains why the reference-aligned pin did not alter native draft tokens. |
...
- XR54 aligns the MTP drafter position argument with Hugging Face's
  ...
  accepted acceptance fix: measured draft tokens were byte-identical to XR48 and
  `mtp_candidate_1k_001` slot-1 acceptance stayed `3/18`. Treat pre-XR54 MTP
  acceptance numbers as historical, but do not start XR55 or broader MTP
  performance work until the drafter-only PyTorch parity contingency explains
  why the position pin did not affect native drafts.
