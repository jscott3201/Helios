...
| Position-pinned MTP drafter | `chat_short_1k_001` | `3/3` | `69/96 = 0.719` | `2956.027 -> 2421.899 ms` (`+18.069%`) | Needs more data; draft tokens unchanged vs XR48 | XR54 |
| Position-pinned MTP drafter | `tool_json_1k_001` | `3/3` | `75/96 = 0.781` | `2955.781 -> 2205.883 ms` (`+25.371%`) | Needs more data; draft tokens unchanged vs XR48 | XR54 |
| Position-pinned MTP drafter | `mtp_candidate_1k_001` | `3/3` | `21/45 = 0.467` | `2883.842 -> 2987.766 ms` (`-3.604%`) | Prediction refuted; slot 1 stayed `3/18` | XR54 |
...
| 2026-07-03 | XR54 MTP drafter position pin | Blocked with evidence | local XR54 position-pin plus XR54-R provenance/parity changes | `native_mtp_position_pin_xr48_config` | `benchmarks/out/XR54-mtp-position-pin/{rung10-native-mtp,pinned-xr48-config,xr54-r-mtp-candidate-one-trial,pytorch-parity}` plus `codex/goals/XR54-mtp-position-pin.goal.md` | Changed `NativeMtpAssistantModel::draft_block` to pass constant `first_position` to every assistant step, matching Hugging Face `SinglePositionMultiTokenCandidateGenerator` constant `position_ids` behavior. The original XR48-config holdout completed with `12/12` exact records and unchanged XR48 draft-token arrays, but review forensics questioned binary freshness. XR54-R rebuilt from cleaned `target/` with `GEMMA4D_REQUIRE_MLX=1`; `CMakeCache.txt` recorded `GEMMA4D_REQUIRE_MLX:BOOL=ON`, native objects/archive and `target/debug/examples/xr15_mtp_policy_variance_ab` all postdated the `00:26:39` source pin edit. The fresh one-leg rerun `xr54-r-mtp-candidate-one-trial` stamped git SHA `f2fb705706bc8196845b19d01170cb41e04f430f`, dirty-diff SHA-256 `b4eae5c622bd802783ba2ca18b3b15f108b5fa615626a2283745849891451bd7`, and runner link mtime `1783054369`; it was exact, accepted `7/15 = 0.467`, and its draft arrays were byte-identical to all XR48 measured `mtp_candidate_1k_001` records. Therefore the position-pin acceptance prediction is genuinely refuted: per-slot acceptance remains `chat_short_1k_001` slot0 `36/60`, slot1 `33/36`; `tool_json_1k_001` slot0 `39/57`, slot1 `36/39`; `mtp_candidate_1k_001` slot0 `18/27`, slot1 `3/18`. Added mandatory build-provenance stamping to XR15 evidence records and summaries. Added `gemma4_kv_snapshot_save_mtp_parity`, Rust wrapper, `xr54_drafter_pytorch_parity`, and `scripts/xr54_drafter_pytorch_parity.py`; the diagnostic exported `payload.safetensors` with `hidden.last`, shared KV, and `target.token_embeddings` shaped `1x2x3840` for token IDs `107,236792`, and native draft `[236792,236865]` matched XR54-R. The actual vendored-Transformers PyTorch comparison is blocked locally because `/opt/homebrew/opt/mlx-lm/libexec/bin/python` has no `torch` module. XR55 must not start until the PyTorch parity comparison runs or a replacement reference path explains why the pin does not alter native drafts. |
...
| 2026-07-03 | Added XR54-R build-provenance stamping and the XR54 drafter-only parity diagnostic. XR15 summaries/records now fail closed unless git SHA, dirty-diff SHA-256, dirty-diff byte count, runner binary path, and runner binary link mtime are available. The new parity path exports `hidden.last`, shared KV, and ordered target token embeddings for vendored-Transformers comparison without changing normal snapshot semantics. | `crates/gemma4d-bench/examples/xr15_mtp_policy_variance_ab.rs`, `native/gemma4_mlx/include/gemma4_mlx.h`, `native/gemma4_mlx/src/native_model.{h,cc}`, `native/gemma4_mlx/src/runtime.cc`, `crates/gemma4d-ffi/src/lib.rs`, `crates/gemma4d-bench/examples/xr54_drafter_pytorch_parity.rs`, `scripts/xr54_drafter_pytorch_parity.py`, `codex/goals/XR54-mtp-position-pin.goal.md`, `BENCHMARKS.md` | `cargo fmt --all --check`; `GEMMA4D_REQUIRE_MLX=1 cargo build -p gemma4d-bench --example xr15_mtp_policy_variance_ab`; `GEMMA4D_REQUIRE_MLX=1 cargo build -p gemma4d-bench --example xr54_drafter_pytorch_parity`; `GEMMA4D_REQUIRE_MLX=1 GEMMA4D_USE_NATIVE_GRAPH=1 GEMMA4D_EXPERIMENTAL_MTP_LAZY_SECOND_DRAFT=1 target/debug/examples/xr54_drafter_pytorch_parity --out-dir benchmarks/out/XR54-mtp-position-pin/pytorch-parity --reference-records benchmarks/out/XR54-mtp-position-pin/xr54-r-mtp-candidate-one-trial/records.jsonl` (blocked with evidence: selected Python has no `torch`). |
...
| 2026-07-03 | `GEMMA4D_REQUIRE_MLX=1 GEMMA4D_USE_NATIVE_GRAPH=1 GEMMA4D_EXPERIMENTAL_MTP_LAZY_SECOND_DRAFT=1 target/debug/examples/xr54_drafter_pytorch_parity --out-dir benchmarks/out/XR54-mtp-position-pin/pytorch-parity --reference-records benchmarks/out/XR54-mtp-position-pin/xr54-r-mtp-candidate-one-trial/records.jsonl` | Blocked with evidence | Escalated Metal/MLX run exported `payload.safetensors`; native draft `[236792,236865]` matched XR54-R and payload metadata includes `target.token_embeddings` for `107,236792`. The PyTorch reference comparison is blocked because the selected Python environment has no `torch` module. |
...
- XR54 aligns the MTP drafter position argument with Hugging Face's
  constant-position invariant and preserves exactness, but it is not an
  ...
  acceptance stayed `3/18`. XR15 evidence records now stamp git SHA,
  dirty-diff SHA-256, dirty-diff byte count, runner binary path, and runner
  binary link mtime. Treat pre-XR54 MTP acceptance numbers as historical, but do
  not start XR55 or broader MTP performance work until the drafter-only PyTorch
  parity comparison runs; the current parity payload export is complete, but
  local execution is blocked because the selected Python lacks `torch`.
