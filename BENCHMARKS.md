...
| QAT target pairing smoke | `chat_short_1k_001` | `2/2` | `0/2 = 0.000` | `19713.684 -> 52344.264 ms` (`-165.522%`) | Blocked; low-N smoke only, not defaultable | XR50 |
| QAT target pairing smoke | `mtp_candidate_1k_001` | `1/1` | `2/2 = 1.000` | `13510.088 -> 25448.830 ms` (`-88.369%`) | Blocked; low-N smoke only, not defaultable | XR50 |
...
| 2026-07-02 | XR50 QAT target MTP pairing A/B | Blocked with evidence | `fe358c8` plus local XR50 goal/docs | `native_mtp_qat_target_pairing_smoke` | `benchmarks/out/XR50-qat-target-mtp-pairing/{report.md,decision.md,blockers.md}` plus smoke subdirectories | Downloaded `mlx-community/gemma-4-12B-it-qat-4bit` revision `e70c6b3ba0979b3357dcd2f223ad8bde7787a6b6` to `artifacts/models/gemma-4-12B-it-qat-4bit`; config SHA-256 `fe091f98e6f7e5e80461bd8ec7ced6d87ac16987586239386ed44b82ecbc2b12`, tokenizer SHA-256 `cc8d3a0ce36466ccc1278bf987df5f71db1719b9ca6b4118264f45cb627bfe0f`, safetensors inventory SHA-256 `fc6c056d37f941612ade5fe8632713471126b599f8bb86eaeb78e3773c2f1358`. QAT config validated as Gemma4 unified, 48 layers, hidden 3840, head dims 256/512, KV heads 8/1, softcap 30.0, mixed affine 4-bit g64 plus MLP 8-bit overrides. `GEMMA4D_REQUIRE_MLX=1 cargo check -p gemma4d-bench --example xr15_mtp_policy_variance_ab` passed. Escalated QAT smoke `candidate-qat-target-block12-smoke` used `--trials 1 --warmups 0 --max-new-tokens 2 --block-sizes 1,2 --workload-id chat_short_1k_001` and was exact `2/2`, but acceptance was `0/2` for both block sizes and MTP decode-phase regressed by `-261.573%`/`-165.522%`; peak MLX about `11.90 GB`. Escalated QAT smoke `candidate-qat-target-mtp-candidate-1k-smoke` used `--trials 1 --warmups 0 --max-new-tokens 2 --block-sizes 2 --workload-id mtp_candidate_1k_001` and was exact `1/1` with acceptance `2/2`, but native decode `13510.088 ms` vs MTP decode phase `25448.830 ms` (`-88.369%`). Broader 3-workload and 32-token QAT selected-path attempts were stopped before artifacts because they ran too long and caused heavy 16GB memory pressure. Do not promote QAT target from XR50; next useful work remains verifier/runtime cost. |
