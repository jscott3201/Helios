...
| 2026-07-01 | XR11 persistent native/server backend A/B | Passed | `8e7dd1b` plus local XR11 changes | `server_real_helper_vs_persistent_native_real_contexts` | `benchmarks/out/XR11-persistent-native-server-ab/{records.jsonl,summary.json,report.md,blockers.md,decision.md}` | Run ID `xr11-1782887715`; command `GEMMA4D_EXPERIMENTAL_PERSISTENT_SERVER=1 cargo run -p gemma4d-bench --example xr11_persistent_native_server_ab -- --out-dir benchmarks/out/XR11-persistent-native-server-ab --model-path artifacts/models/gemma-4-12B-it-4bit --workloads benchmarks/workloads/real-contexts/workloads.jsonl --workload-ids chat_short_1k_001 --repeats 2 --max-new-tokens 1`; workload seed `20260630`, target/actual context `1024/1024`, generated `1` token per repeat. Baseline `real_helper` loaded twice; `persistent_native` loaded once and served two worker requests. Both repeats matched token IDs/text; no blockers. Initial sandbox run was blocked by localhost bind permission and was rerun with approval. |
