...
| Real MTP top-k/margin instrumentation, enabled | 1K family, `N=2/4` | `18/18` measured | unchanged | vs 3-run main mean: N=2 `+1.916%`, N=4 `+0.925%` selected-decode delta | `GEMMA4D_EXPERIMENTAL_MTP_REAL_MARGINS=1`; trace spot-check and PyTorch pinned score parity passed | XR57 |
...
| 2026-07-03 | XR57 real MTP top-k and margin instrumentation | Accept candidate, env-gated | `b9e2dbe` plus local XR57 ABI-v4 instrumentation | `native_mtp_real_margins_topk` | `benchmarks/out/XR57-real-margins/{baseline-main-rerun*-n2n4-abs,candidate-gated-tokenonly-cached-n2n4,candidate-real-margins-enabled-final-n2n4,drafter-score-parity-final,xr57-trace-spotcheck-final.json}` plus `codex/goals/XR57-real-margins.goal.md` | Replaced fake MTP score fields with real target top-k and drafter score plumbing behind `GEMMA4D_EXPERIMENTAL_MTP_REAL_MARGINS=1`, widened the native draft ABI to v4, added Rust `DraftToken`/`draft_block_with_scores`, and kept a token-only default path so disabled instrumentation does not pay score-buffer bookkeeping. Fixed trace row reuse so cached lookahead steps copy the source top-k row matching `step.sequence_len - 1` rather than stale slot 0. The default candidate run `xr15-1783103456` wrote `24/24` exact records (`18/18` measured), `trace_top_k=[1]`, zero draft score fields, and no target-top1 mismatches; versus the three clean `b9e2dbe` baselines, selected-decode delta was N=2 `-1.034%` and N=4 `+0.777%` (fastest single N=4 baseline would show `+2.945%`, so XR57 records the variance caveat). Enabled run `xr15-1783104010` wrote `24/24` exact records with `trace_top_k=[5]`; `scripts/xr57_trace_spotcheck.py` passed on 3 recorded events, and `xr54_drafter_pytorch_parity` run `xr54-parity-1783104461` completed with pinned native/PyTorch tokens matching and pinned drafter logits/margins within `0.5` bf16 tolerance. Enabled real-margin overhead versus the three-run main mean was N=2 `+1.916%`, N=4 `+0.925%`; therefore real top-5/margin capture remains env-gated and adaptive-N logic remains wave-5 only. Pre-XR57 top-k/margin fields are historical-fake except target top-1/greedy fields. |
