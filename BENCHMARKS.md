...
|---|---|---:|---:|---:|---|---|---|
| Server default sentinel | `long_repo_pack_24k_001` | `60939.960` | `61254.060` | `-0.515%` | `7.859 -> 7.859 GB` | Accepted default-wiring sentinel; explicit native vs default native, `2/2` token identity, low-N edge run, policy applied | XR62 `server-default-sentinel-24k-low-n` |
| Server default sentinel | `benchmark_qa_16k_001` | `43289.598` | `41360.009` | `+4.457%` | `7.639 -> 7.639 GB` | Accepted default-wiring sentinel; explicit native vs default native, `3/3` token identity, policy applied | XR62 `server-default-sentinel-16k` |
| Server default sentinel | `code_review_rust_8k_001` | `20289.683` | `20587.685` | `-1.469%` | `7.402 -> 7.402 GB` | Accepted default-wiring sentinel; explicit native vs default native, `3/3` token identity, policy applied | XR62 `server-default-sentinel-8k` |
...
| Native decode stage profile | `chat_short_1k_001` + `tool_json_1k_001` | `64 x 6` | n/a | `81.216..81.341` | `82.636..84.410` | Profiling accepted; no native decode behavior/default change; dominant lane is `forward_graph_ms` at `76.012..79.996 ms` mean, FFI/greedy/telemetry means are sub-ms | XR64 `native-decode-profile` |
| Native decode KV eval default | `chat_short_1k_001` + `tool_json_1k_001` | `64 x 12` | n/a | `81.401 -> 70.621`; `81.120 -> 70.614` | `85.989 -> 72.425`; `82.277 -> 73.170` | Accepted; runtime default now end-of-decode, explicit per-layer remains env-selectable | XR65 `runtime-default-ab` |
| Native KV materialization profile | selected 1K/4K/8K/16K | `64 x 15` | n/a | `69.593..76.783` | `70.688..88.492` | Profiling complete; runtime-default deferred KV eval is dominated by full-attention materialization (`63.118..77.984 ms` mean), sliding eval is `0.006..0.009 ms`; no default change | XR69 `baseline-deferred-kv-split` |
| Native full-attention KV update | selected 1K/4K/8K/16K | `64 x 45` | n/a | `69.773..77.189 -> 70.027..73.287` | `74.754..87.077 -> 76.697..79.699` | Accepted as default-off candidate; total decode `78020.982 -> 73634.458 ms` (`+5.622%`), full-attn deferred eval improved 4/5 rows but `chat_short_1k_001` regressed; defaults unchanged | XR70 `full-matrix` |
| Native full-attention KV update capacity | selected 1K/4K/8K/16K | `64 x 45` | n/a | `70.214..78.036 -> 68.655..71.426` | `73.522..90.276 -> 70.968..77.321` | Useful but not clean enough for promotion; total decode `73947.065 -> 68583.341 ms` (`+7.253%`), 16K peak `7.929 GB`, update overhead only about `0.010 ms/token`, but chat p95/p99 regressed beyond the 5% guardrail and only 1/5 rows cleared the candidate tail gate; MTP side-effect rerun skipped | XR71 `full-matrix-256` |
| Native full-attention group profile | selected 1K/4K/8K/16K | `64 x 45` | n/a | runtime default `72.251..78.750` | runtime default `74.935..86.754` | Profiling complete; runtime default accepted vs per-layer on all five rows, collection time was `0.006..0.009 ms` mean and not the tail source, chat first-token outliers were dominated by full-attention group eval, and the `256` update candidate did not beat runtime default; defaults unchanged | XR72 `full-matrix` |
| Native full-attention group-eval experiment | `chat_short_1k_001` | `64 x 9` | n/a | `69.655 -> 71.448` | `70.191 -> 72.057` | Rejected vs runtime default; exactness clean `9/9`, peak `7.321 GB`, but serial group eval regressed p50 by `2.574%`, regressed p95 by `2.659%`, and improved p99 only `0.602%`; no full matrix warranted | XR75 `chat-1k-followup` |
| Native first-token warmup probe | `chat_short_1k_001` | `64 x 9` | n/a | `69.778 -> 70.460` | `70.367 -> 71.690` | Accepted as hypothesis evidence only; warmup cost excluded from measured record, exactness clean `9/9`, peak `7.321 GB`, raw p99 `177.571 -> 86.680` (`+51.186%`), first-token p50 `177.571 -> 86.680`; next step is a real default-off warmup policy candidate | XR76 `warmup-probe-chat-1k` |
| Native first-token warmup costed | `chat_short_1k_001` | `64 x 9` | n/a | `69.499 -> 69.643` | `70.740 -> 70.977` | Accepted as cost-accounted hypothesis evidence only; exactness clean `9/9`, peak `7.321 GB`, raw p99/max `188.836 -> 92.922` (`+50.792%`), first-token p50 `188.836 -> 92.922`, but discarded warmup total p50 was `3203.529 ms` (`2737.664 ms` prefill + `360.509 ms` decode), so this is viable only as out-of-request/load-time or amortized shape warmup unless broader evidence changes the cost model; defaults unchanged | XR77 `costed-chat-1k` |
| Native amortized first-token warmup | `chat_short_1k_001` + `code_review_rust_4k_001` | `32 x 36` | n/a | chat `70.524 -> 70.300`; code `72.319 -> 72.424` | chat `72.888 -> 71.508`; code `73.822 -> 73.691` | Accepted for chat tail only; exactness clean `36/36`, peak `7.639 GB`, chat raw p99/max `387.059 -> 92.292` (`+76.155%`) and first-token p50 `387.059 -> 92.292`, while 4K did not reproduce a tail; warmup event p50 was `3843.020 ms` chat and `9272.415 ms` code, amortized over 4 requests to `960.755 ms` and `2318.104 ms`, so this remains an out-of-request/load-time shape warmup lane; defaults unchanged | XR78 `amortized-1k-4k-trials3` |
...
| Real MTP top-k/margin instrumentation, default path | 1K family, `N=2/4` | `18/18` measured | unchanged | vs 3-run main mean: N=2 `-1.034%`, N=4 `+0.777%` selected-decode delta | `accept_candidate` as env-gated instrumentation; default records top-1/zero score fields | XR57 |
| Real MTP top-k/margin instrumentation, enabled | 1K family, `N=2/4` | `18/18` measured | unchanged | vs 3-run main mean: N=2 `+1.916%`, N=4 `+0.925%` selected-decode delta | real margins/top-k only with `GEMMA4D_EXPERIMENTAL_MTP_REAL_MARGINS=1`; trace spot-check and PyTorch score parity passed | XR57 |
| Real MTP top-k/margin instrumentation holdout | `mtp_candidate_1k_001`, `N=2/4` | `6/6` measured | N=2 `21/45 = 0.467`; N=4 `21/69 = 0.304` | default N=2 `-0.569%`, N=4 `-5.412%`; enabled N=2 `-0.941%`, N=4 `-4.364%` | Own XR57 holdout row; auto-disabled `6/6`, not selected | XR57 |
| Adaptive-N MTP safe-bypass policy | `chat_short_1k_001:adaptive` + `tool_json_1k_001:adaptive` | `9/9` measured | `144/204 = 0.706` selected | aggregate `8315.953 -> 6544.437 ms` (`+21.303%`) | `keep_experimental`, default-off; holdout protected and oracle matched, but below 25% default-on gate | XR61 |
| Post-XR65 Adaptive-N MTP revisit | `chat_short_1k_001:adaptive` + `tool_json_1k_001:adaptive` plus protected `mtp_candidate_1k_001` | `9/9` measured; oracle `9/9` | `144/204 = 0.706` selected | protected aggregate `7904.818 -> 6297.468 ms` (`+20.334%`); selected lanes `5179.439 -> 3572.089 ms` (`+31.033%`) | `keep_experimental`, default-off; faster native default lowers broad aggregate, still below 25% default-on gate | XR66 |
| Post-XR70 Adaptive-N MTP revisit | `chat_short_1k_001:adaptive` + `tool_json_1k_001:adaptive` plus protected `mtp_candidate_1k_001` | `9/9` measured; oracle `9/9` | `144/204 = 0.706` selected | protected aggregate `7474.609 -> 5991.287 ms` (`+19.845%`); selected lanes `4835.648 -> 3347.089 ms` (`+30.784%`) | `keep_experimental`, default-off; full-attn KV update does not close the 25% broad gate | XR70 |
| Scoped MTP chat/tool opt-in | `chat_short_1k_001:adaptive` + `tool_json_1k_001:adaptive` plus protected 1K/4K holdouts | candidate `12/12`; oracle `9/9`; default-off `12/12`; 4K holdout `12/12` | `144/204 = 0.706` selected | protected aggregate `7523.808 -> 6076.627 ms` (`+19.235%`); selected lanes `+28.820%`; default-disabled overhead `-0.000%` | Accepted for explicit scoped opt-in only; broad default-on rejected because protected aggregate remains below 25%; no runtime default change | XR73 |
| Warmup-aware MTP protected aggregate | `chat_short_1k_001:adaptive` + `tool_json_1k_001:adaptive` plus protected 1K/4K holdouts | candidate `12/12`; oracle `9/9`; default-off `12/12`; 4K holdout `12/12` | `144/204 = 0.706` selected | protected aggregate `7479.958 -> 6022.716 ms` (`+19.482%`); selected lanes `+29.237%`; default-disabled overhead `-0.000%` | Accepted for scoped evidence only; broad default-on still rejected because protected aggregate remains below 25%; XR78 native warmup remains out-of-request/load-time context only; no runtime default change | XR79 |
| MTP protected aggregate overhead attribution | XR79 selected chat/tool plus protected aggregate | source candidate `12/12`; measured `9/9` | `144/204 = 0.706` selected | broad gate gap `412.747 ms`; selected target `3527.062 -> 3114.315 ms`; verifier forward `3027.162 ms` (`85.827%` selected phase); first verifier-pass excess `497.460 ms` | `needs_runtime_candidate`; no runtime/default change; next target is first verifier-forward warm/JIT/cache isolation while preserving holdouts | XR81 `xr81-mtp-overhead-gap` |
| Terminal block-prefix no-lookahead | `chat_short_1k_001:adaptive` + `tool_json_1k_001:adaptive` | `6/6` measured; oracle `6/6` | `144/204 = 0.706` selected | P3 selected decode `3822.879 -> 3744.794 ms` (`+2.043%`) | `reject_candidate`; correct but below the `+5%` P3 patch gate, terminal skips `0 -> 6`, default-off | XR63 |
| Adaptive-N MTP safe-bypass holdout | 4K holdouts and protected MTP candidate | `9/9` measured | `0/0`; all baseline-bypassed | aggregate `9755.257 -> 9755.257 ms` (`+0.000%`) | Holdout protection pass; no MTP selected for unproven/protected workloads | XR61 |
| Adaptive-N MTP generic holdout probe | 4K holdouts under generic policy | `6/9` measured | generic adaptive `N=2` on `code_review_rust_4k_001` | no promoted speed claim | Blocked/rejected; code-review exactness failed, causing safe bypass for unknown workload families | XR61 v1 |
| Repair-sliced N-block MTP guarded policy | `chat_short_1k_001:N=3` + `tool_json_1k_001:N=6` | `54/54` measured | `144/204 = 0.706` selected | aggregate `8458.990 -> 6598.914 ms` (`+21.989%`) | `keep_experimental`, default-off; below 25% default-on gate; sequential oracle matched `72/72` records | XR56 |
| Repair-sliced N-block MTP holdout | `mtp_candidate_1k_001` | `18/18` | `132/441 = 0.299` | best fixed N `+0.380%`; N=8 `-23.759%` | Not selected; auto-disabled `18/18` measured records | XR56 |
| Repair-sliced MTP fixed block | 1K family, `N=4` | `9/9` | `162/270 = 0.600` | aggregate `+17.609%` | Improved vs XR55 `+7.151%`; repair forward `0.0 ms`, fallback remains | XR56 |
...
| N-block MTP sweep holdout | `mtp_candidate_1k_001` | `18/18` | `132/441 = 0.299` | best fixed N `-0.770%`; N=8 `-26.522%` | Not selected; auto-disabled `18/18` measured records | XR55 |
...
| 2026-07-03 | XR55 MTP N-block generalization | Keep experimental | local XR55 native MTP N-block changes | `native_mtp_nblock_block_prefix_verify` | `benchmarks/out/XR55-nblock-generalization/{baseline-block2,trace-n8-chat-prefix-repair,candidate-nblock-sweep,sequential-oracle-sweep,xr55-nblock-summary.md,xr55-nblock-summary.json}` plus `codex/goals/XR55-nblock-generalization.goal.md` | Generalized the default-off native MTP block-prefix experiment beyond block 2 by widening MTP trace/committed-token capacity to 16 positions, accepting draft blocks up to the trace capacity, making trace overflow fail closed, bumping native ABI version to `2`, and repairing later partial accepts by materializing the accepted-prefix KV before fallback decode. Existing XR55 legs used the intended XR48-style env config (`GEMMA4D_REQUIRE_MLX=1`, `GEMMA4D_USE_NATIVE_GRAPH=1`, `GEMMA4D_EXPERIMENTAL_MTP_BLOCK_PREFIX_ROLLBACK=1` where applicable, `GEMMA4D_EXPERIMENTAL_MTP_LAZY_SECOND_DRAFT=1`); active `GEMMA4D_*` env stamping was added after these records, so the historical XR55 `build_provenance` lacks `gemma4d_env` and future runs will stamp it. Baseline post-XR54 `main@24186cf` block 2 selected `chat_short_1k_001` and `tool_json_1k_001` with aggregate `16.038%` speedup. The final candidate sweep used `{1,2,3,4,6,8}` with 3 measured trials plus 1 warmup over `chat_short_1k_001`, `tool_json_1k_001`, and `mtp_candidate_1k_001`; all `72/72` records were exact against non-MTP greedy, and the separate sequential-oracle sweep matched generated tokens for all `72/72` records. Fixed blocks: N=1 `-8.266%`, N=2 `+14.168%`, N=3 `+18.054%`, N=4 `+7.151%`, N=6 `-1.367%`, N=8 `-9.353%`; `tokens/verify` in the XR55 summary excludes auto-disabled fallback records and is `1.000/1.641/2.065/2.783/3.048/3.200`. `mtp_candidate_1k_001` was exact `18/18` but regressed at every fixed N (best N=2 `-0.770%`, N=8 `-26.522%`) and auto-disabled in `18/18` measured records. The net-latency guard selected `chat_short_1k_001:N=3` plus `tool_json_1k_001:N=4` with aggregate `20.371%` speedup, weighted acceptance `144/198 = 0.727`, and peak MLX `8.357 GB`; unselected workloads enter that aggregate at baseline decode on both sides. Sequential commit semantics are frozen by the oracle; trace and ABI plumbing may change only with differential generated-token evidence. Per-slot acceptance drops after slot 3; N>=4 pays large exact-prefix repair (`verify_repair_ms` around `6117..6270 ms` aggregate). Draft cost per draft step stayed below the `0.1` verify-unit flag threshold for all blocks. MTP remains default-off. |
| 2026-07-03 | XR56 MTP repair-path cost removal | Keep experimental | `06a2e1a` plus ledger docs | `native_mtp_nblock_retro_prefix_repair` | `benchmarks/out/XR56-repair-cost/{baseline-main,candidate-retro-prefix,sequential-oracle-sweep,xr56-repair-cost-summary.md,xr56-repair-cost-summary.json}` plus `codex/goals/XR56-repair-cost.goal.md` | Removed the redundant second full block forward from accepted-prefix repair by capturing first-forward block KV sources and retroactively slicing the accepted-prefix KV before the fallback decode. Split verifier repair timing into `repair_clone_ms`, `repair_forward_ms`, and `repair_fallback_ms`, widened the FFI result ABI to `3`, and extended the XR15 report with repair subcolumns plus a mandatory `build_provenance.gemma4d_env` gate. Fresh main baseline `fa50bd0` used the XR55 sweep config with active env stamped and clean provenance; run `xr15-1783069298` wrote `72/72` exact records, selected `chat_short_1k_001:N=3` plus `tool_json_1k_001:N=4`, and measured aggregate `20.163%`. Candidate run `xr15-1783069895` from clean `06a2e1a` wrote `72/72` exact records with no hard blockers; the sequential-oracle run `xr15-1783070446` matched generated tokens for `72/72` records. Fixed blocks were N=1 `-7.195%`, N=2 `+16.127%`, N=3 `+20.320%`, N=4 `+17.609%`, N=6 `+9.013%`, N=8 `+2.116%`; N=4 improved from XR55 `+7.151%` to `+17.609%`, while N=6/N=8 improved but remained acceptance-bound. The guarded selector chose `chat_short_1k_001:N=3` and `tool_json_1k_001:N=6`; tool JSON's N=6 pick is within noise of N=3/N=4 at 3 measured trials, so wave-5 should not overfit that exact block choice. Candidate repair sub-timers showed `repair_forward_ms=0.0` across measured records; remaining `verify_repair_ms` is fallback decode (`repair_fallback_ms`) rather than dead prefix re-forward. `mtp_candidate_1k_001` has its own row: exact `18/18`, best fixed N=3 `+0.380%`, N=8 `-23.759%`, auto-disabled `18/18`, and not selected. The guarded aggregate was `8458.990 -> 6598.914 ms` (`+21.989%`), weighted selected acceptance `144/204 = 0.706`, peak selected MLX `8.004 GB`; unselected workloads enter that aggregate at baseline decode on both sides. This is below the `25%` default-on gate, so MTP remains default-off. |
| 2026-07-03 | XR57 real MTP top-k and margin instrumentation | Accept candidate, env-gated | `b9e2dbe` plus local XR57 ABI-v4 instrumentation | `native_mtp_real_margins_topk` | `benchmarks/out/XR57-real-margins/{baseline-main-rerun*-n2n4-abs,candidate-gated-tokenonly-cached-n2n4,candidate-real-margins-enabled-final-n2n4,candidate-real-margins-enabled-spotcheck-final,drafter-score-parity-final,xr57-target-logits-anchor-final.json,xr57-trace-spotcheck-final.json}` plus `codex/goals/XR57-real-margins.goal.md` | Replaced fake MTP score fields with real target top-k and drafter score plumbing behind `GEMMA4D_EXPERIMENTAL_MTP_REAL_MARGINS=1`, widened the native draft ABI to v4, added Rust `DraftToken`/`draft_block_with_scores`, and kept a token-only default path so disabled instrumentation does not pay score-buffer bookkeeping. Fixed trace row reuse so cached lookahead steps copy the source top-k row matching `step.sequence_len - 1` rather than stale slot 0, and degrade honestly to `trace_top_k=1` if only rank 0 is populated. The default candidate run `xr15-1783103456` wrote `24/24` exact records (`18/18` measured), `trace_top_k=[1]`, zero draft score fields, and no target-top1 mismatches; versus the three clean `b9e2dbe` baselines, selected-decode delta was N=2 `-1.034%` and N=4 `+0.777%` (fastest single N=4 baseline would show `+2.945%`, so XR57 records the variance caveat). Enabled run `xr15-1783104010` wrote `24/24` exact records with `trace_top_k=[5]`; review-round spot-check run `xr15-1783108188` wrote `18/18` exact measured records with `trace_top_k=[5]`, exported a raw target-logits anchor, and `scripts/xr57_trace_spotcheck.py` passed with `237/237` checked events clean, rejected/accepted/full-accept coverage, and raw-logit top-5 ids `[45518,236779,236787,1904,236751]` matched to a recorded target row. `xr54_drafter_pytorch_parity` run `xr54-parity-1783108431` completed with pinned and incremented native/PyTorch draft tokens matching; pinned drafter logits matched within `0.5` and margins within `0.25`. enabled-path guard failed at N=2 (+1.916% > 1%) → env-gated per rule; N=4 was `+0.925%`, and adaptive-N logic remains wave-5 only. `mtp_candidate_1k_001` has its own XR57 row: default path N=2 `-0.569%`, N=4 `-5.412%`; enabled path N=2 `-0.941%`, N=4 `-4.364%`; auto-disabled `6/6`, not selected. Pre-XR57 top-k/margin fields are historical-fake except target top-1/greedy fields, and XR57 margins/top-k beyond top-1 are real only under `GEMMA4D_EXPERIMENTAL_MTP_REAL_MARGINS=1`. |

| 2026-07-04 | XR61 adaptive-N MTP safe-bypass policy | Keep experimental | `3195450` plus ledger docs | `native_mtp_adaptive_n_safe_bypass` | `benchmarks/out/XR61-adaptive-n-mtp/{candidate-adaptive-n-v2-safe-bypass,candidate-adaptive-n-v2-safe-bypass-holdouts,sequential-oracle-adaptive-n-v2-safe-bypass,xr61-adaptive-n-summary.md,xr61-adaptive-n-summary.json}` plus `codex/goals/XR61-adaptive-n-mtp.goal.md` | Added a gate-aware adaptive-N report and a conservative XR15 harness policy `xr61-real-margin-v1` that runs live adaptive MTP only on the proven `chat_short_1k_001` and `tool_json_1k_001` lanes while baseline-bypassing protected `mtp_candidate_*` and unproven holdout workloads. The primary v2 candidate run `xr15-1783193438` used `GEMMA4D_REQUIRE_MLX=1`, native graph, block-prefix rollback, lazy second draft, real margins, and adaptive-N over block sizes `1,2,3,4,6,8`; it wrote `12` records (`9` measured) with `9/9` measured exactness, selected the two proven lanes, measured aggregate `8315.953 -> 6544.437 ms` (`+21.303%`), weighted acceptance `144/204 = 0.706`, and peak MLX `8.008 GB`. The v2 full 4K holdout run `xr15-1783193613` wrote `12` records (`9` measured) with `9/9` measured exactness, selected no MTP workloads, and baseline-bypassed `code_review_rust_4k_001`, `benchmark_qa_4k_001`, and `mtp_candidate_4k_001`, so holdout aggregate remained `+0.000%` with peak `9.244 GB`. The sequential-oracle run compared `9` measured candidate records with no missing/extra/mismatched generated-token records. The earlier v1 generic holdout probe is retained as blocker evidence: `code_review_rust_4k_001` failed exactness under generic adaptive `N=2` (`6/9` measured exact overall), so unknown workload families must not be auto-promoted. Default-on gates are false because speed is below `25%`, default-path overhead was not remeasured, and risk review is still unrecorded. XR61 closes P1 as env-gated/harness-only; the remaining limiter is accepted tokens per verifier/fallback cost rather than DSpark. |
| 2026-07-04 | XR62 server default sentinel closure | Accept candidate | `d4e4405` plus ledger docs | `server_explicit_persistent_native_vs_default_persistent_native` | `benchmarks/out/XR61-adaptive-n-mtp/{server-default-sentinel-8k,server-default-sentinel-16k,server-default-sentinel-24k-low-n}` plus `codex/goals/XR62-server-default-sentinel-closure.goal.md` | Closed P2 native server/default observability by adding structured server-side prefill policy state to `/v1/runtime/snapshot` (`status`, `policy`, `reason`, `warning`) and adding `/v1/config.native_prefill` policy hints without widening the FFI boundary. Extended `xr11_persistent_native_server_ab` with `--baseline-backend persistent-native` so long-context sentinels compare explicit PersistentNative against the default model-path PersistentNative path; this avoids the post-XR53 RealHelper unchunked admission fail-closed path. Validation passed: `cargo fmt --all --check`; `git diff --check`; `cargo test -p gemma4d-server --all-targets`; `cargo test -p gemma4d-bench --example xr11_persistent_native_server_ab --no-run`; `GEMMA4D_REQUIRE_MLX=1 cargo check -p gemma4d-bench --example xr11_persistent_native_server_ab`. Escalated Metal runs passed: `code_review_rust_8k_001` `3/3` token identity, policy `applied -> applied`, prefill p50/p95 `20289.683/20463.171 -> 20587.685/22733.979 ms`, peak `7.402 -> 7.402 GB`; `benchmark_qa_16k_001` `3/3`, p50/p95 `43289.598/46118.003 -> 41360.009/43759.908 ms`, peak `7.639 -> 7.639 GB`; `long_repo_pack_24k_001` low-N `2/2`, p50/p95 `60939.960/68970.317 -> 61254.060/64120.647 ms`, peak `7.859 -> 7.859 GB`. This is default-wiring/safety evidence, not a new prefill speed claim. P2 is accepted; next work should target MTP verifier/fallback cost. |
| 2026-07-04 | XR63 MTP terminal block-prefix no-lookahead | Reject candidate | `44359fb` | `native_mtp_terminal_block_prefix_no_lookahead` | `benchmarks/out/XR61-adaptive-n-mtp/{p3-terminal-block-prefix-baseline,p3-terminal-block-prefix-candidate,p3-terminal-block-prefix-sequential-oracle,p3-terminal-block-prefix-summary.md,p3-terminal-block-prefix-summary.json}` plus `codex/goals/XR63-mtp-terminal-block-prefix-no-lookahead.goal.md` | P3 tested a terminal final-block verifier path that keeps the experimental block-prefix verifier active and forwards only `N-1` target tokens when the generation budget ends. The XR15 harness also gained explicit `--allow-missing-source-replay` provenance disclosure because local ignored XR14 replay artifacts were absent. The first sandboxed baseline attempt failed before measurement with `No Metal device available`; the recorded baseline, candidate, and oracle runs were rerun with approved Metal access. Candidate exactness passed `8/8` records (`6/6` measured), candidate-vs-baseline MTP generated tokens matched `6/6`, and candidate-vs-sequential-oracle matched `6/6`. Terminal skips rose `0 -> 6`, weighted acceptance stayed `144/204 = 0.706`, and peak MLX stayed `8.008 GB`, but selected decode improved only `3822.879 -> 3744.794 ms` (`+2.043%`), below the P3 `+5%` patch gate. The path remains benchmark-only/default-off; remaining limiter is verifier/fallback cost and accepted tokens per verifier pass. |
| 2026-07-04 | XR64 native decode stage profile | Profile complete; no valid small cleanup lane | local XR64 profile instrumentation | `native_decode_stage_profile_env_gated` | `benchmarks/out/XR61-adaptive-n-mtp/native-decode-profile/{records.jsonl,summary.json,report.md,blockers.md,decision.md,profile.json,profile.md}` plus `codex/goals/XR64-native-decode-stage-profile.goal.md` | Added env-gated `GEMMA4D_NATIVE_DECODE_PROFILE=1` stage timings to native `decode_incremental`, widened the narrow C/Rust step-result ABI to version `5`, and extended XR06 to emit optional per-token profile traces plus `profile.json`/`profile.md`. Verification passed: `cargo fmt`; `cargo test -p gemma4d-ffi`; `cargo check -p gemma4d-bench --example xr06_native_decode_tail_latency_ab`. The first sandboxed profile command failed before measurement with `No Metal device available`; the recorded escalated Metal run used `GEMMA4D_REQUIRE_MLX=1 GEMMA4D_USE_NATIVE_GRAPH=1 GEMMA4D_NATIVE_DECODE_PROFILE=1 cargo run -p gemma4d-bench --example xr06_native_decode_tail_latency_ab -- --out-dir benchmarks/out/XR61-adaptive-n-mtp/native-decode-profile --trials 3 --max-new-tokens 64 --clear-workload-ids --workload-id chat_short_1k_001 --workload-id tool_json_1k_001 --variant native_decode_eval_per_layer`. It wrote `6/6` passed records and `378/378` profiled decode samples, peak MLX `7.321 GB`, and no blockers. `chat_short_1k_001` host/native means were `86.341/86.331 ms`; `forward_graph_ms` mean `79.996 ms`, `eval_sync_ms` `6.330 ms`, Rust/FFI overhead `0.010 ms`. `tool_json_1k_001` host/native means were `82.292/82.283 ms`; `forward_graph_ms` mean `76.012 ms`, `eval_sync_ms` `6.264 ms`, Rust/FFI overhead `0.009 ms`. Peak reset/read, greedy selection/logit gather, hidden-view handoff, scalar reads, and FFI are not worthwhile near-term targets; the only >5% lane is the broad native forward graph plus decode KV append/cache mutation path, so native decode behavior/defaults stay unchanged pending a scoped native graph/KV follow-up. |
| 2026-07-04 | XR65 native graph/KV mutation lane | Accept candidate | local XR65 split-profile/default changes | `native_decode_runtime_default_end_of_decode` | `benchmarks/out/XR65-native-graph-kv-mutation-lane/{baseline-split-profile,kv-eval-policy-ab,runtime-default-ab}` plus `codex/goals/XR65-native-graph-kv-mutation-lane.goal.md` | Split XR64 `forward_graph_ms` by widening the env-gated decode profile ABI to version `6`: `decode_embedding_ms`, `layer_graph_ms`, `attention_kv_mutation_ms`, `deferred_kv_eval_ms`, `lm_head_ms`, and derived XR06 `non_kv_forward_graph_ms`. Baseline split profile wrote `6/6` passed records and showed the old explicit per-layer path dominated by `attention_kv_mutation_ms`: `chat_short_1k_001` native/forward/KV/non-KV means `85.454/79.156/77.239/1.917 ms`; `tool_json_1k_001` `82.628/76.337/74.433/1.904 ms`. KV-eval policy A/B wrote `24/24` passed records; `end_of_decode` accepted on both workloads with p50 improvements about `14.4%`, p95/p99 gates met, no memory regression, and exactness clean. Runtime-default A/B proved the unset-env path after changing `decode_kv_eval_mode()` default to end-of-decode: `12/12` passed records, `756/756` profiled samples, no blockers, peak MLX `7.321 GB`, and exactness `3/3` per workload. Runtime default vs explicit per-layer improved `chat_short_1k_001` p50/p95 `81.401/85.989 -> 70.621/72.425 ms` with p50 delta `-13.244%` and p95 improvement `15.774%`; `tool_json_1k_001` p50/p95 `81.120/82.277 -> 70.614/73.170 ms` with p50 delta `-12.952%` and p99 improvement `29.720%`. Explicit `GEMMA4D_NATIVE_DECODE_KV_EVAL=per_layer` or `current` retains the old behavior; no MTP default changed. |
| 2026-07-04 | XR66 post-XR65 MTP revisit | Keep experimental | `8253052` plus XR66 contract docs | `xr15_mtp_policy_variance_ab_post_xr65_runtime_default` | `benchmarks/out/XR66-post-xr65-mtp-revisit/{candidate-adaptive-n-post-xr65,sequential-oracle-adaptive-n-post-xr65,xr66-post-xr65-mtp-summary.md,xr66-post-xr65-mtp-summary.json}` plus `codex/goals/XR66-post-xr65-mtp-revisit.goal.md` | Added no runtime code. The primary candidate deliberately left `GEMMA4D_NATIVE_DECODE_KV_EVAL` unset so it used XR65's grouped end-of-decode runtime default, while keeping the XR61 env-gated safe-bypass policy and block-prefix MTP flags. Static gates passed: `cargo fmt --all --check`, `git diff --check`, and `GEMMA4D_REQUIRE_MLX=1 cargo test -p gemma4d-bench --example xr15_mtp_policy_variance_ab --no-run`. The escalated Metal candidate run wrote `12/12` exact records (`9/9` measured), no blockers, peak MLX `8.008 GB`, weighted acceptance `144/204 = 0.706`, and selected `chat_short_1k_001:adaptive` plus `tool_json_1k_001:adaptive`. The XR61-compatible protected aggregate including baseline-bypassed `mtp_candidate_1k_001` moved to `7904.818 -> 6297.468 ms` (`+20.334%`), below the `25%` default-on gate and lower than XR61's `+21.303%`; selected chat/tool lanes alone were `5179.439 -> 3572.089 ms` (`+31.033%`) and are not sufficient for broad default-on. The fresh sequential-oracle run compared `9` measured candidate records with no missing, extra, or mismatched generated-token records. MTP remains default-off/experimental. |
| 2026-07-04 | XR67 native decode deferred-KV eval barrier | Follow-up candidate | `8a39176` plus XR67 contract/report docs | `native_decode_eval_defer_to_logits_vs_runtime_default` | `benchmarks/out/XR67-native-decode-deferred-kv-eval-barrier/{decode-policy-ab,mtp-side-effect-runtime-default,mtp-side-effect-defer-to-logits,xr67-deferred-kv-eval-summary.md,xr67-deferred-kv-eval-summary.json}` plus `codex/goals/XR67-native-decode-deferred-kv-eval-barrier.goal.md` | Added no runtime code and left the XR65 runtime default unchanged. Static gates passed: `cargo fmt --all --check`, `git diff --check`, `python3 -m py_compile scripts/xr67_deferred_kv_eval_report.py`, `GEMMA4D_REQUIRE_MLX=1 cargo test -p gemma4d-bench --example xr06_native_decode_tail_latency_ab --no-run`, and `GEMMA4D_REQUIRE_MLX=1 cargo test -p gemma4d-bench --example xr15_mtp_policy_variance_ab --no-run`. The full XR06 run wrote `75/75` generated/correct records and `4725/4725` profiled decode samples, but returned `blocked_with_evidence` because `benchmark_qa_16k_001` crossed the tiny16 memory cliff at `21.986 GB` for the 16K sentinel. Reinterpreting the clean 1K/4K/8K rows against `native_decode_runtime_default`, `native_decode_eval_defer_to_logits` was the only follow-up candidate: aggregate decode sum `28309.876 -> 20844.570 ms` (`+26.370%`), worst p50/p95/p99 regression `-1.172/-0.682/-1.840%`, peak MLX `12.829 GB`, and all rows exact/memory/tail clean. `native_decode_eval_selective_full_attention` had similar aggregate speed but failed the row gate on `chat_short_1k_001` with p99 regression `27.464%`; per-layer remained a rollback provenance row and failed p50/p95/p99 gates. The profile shows `defer_to_logits` moves the grouped KV work into `eval_sync_ms` rather than removing it. XR15 side-effect probes passed: runtime default and `defer_to_logits` both wrote `12` exact records, selected `chat_short_1k_001:adaptive` plus `tool_json_1k_001:adaptive`, and preserved accepted/attempted draft tokens `312/396` with weighted acceptance `0.787879`; measured aggregate speed was `26.039%` vs `25.280%`. This is evidence for a next scoped/default-off candidate, not a default change. |

| 2026-07-04 | XR68 scoped defer-to-logits policy candidate | Reject candidate | local XR68 env-inheritance/report docs | `native_decode_eval_defer_to_logits_scoped_chunked_vs_runtime_default` | `benchmarks/out/XR68-scoped-defer-to-logits-policy/{decode-policy-chunked-16k,decode-policy-chunked-16k-env-inherited,xr68-defer-to-logits-policy-summary.md,xr68-defer-to-logits-policy-summary.json}` plus `codex/goals/XR68-scoped-defer-to-logits-policy.goal.md` | Added XR06 capture/inheritance for global native prefill/diagnostic env keys so `GEMMA4D_NATIVE_PREFILL_CHUNK_POLICY=long_context_256` survives the benchmark's per-variant `EnvGuard`; the first unpatched XR68 run is retained as blocker evidence because 16K still crossed the unchunked `21.986 GB` peak MLX cliff. The corrected chunked XR06 run wrote `45/45` exact records with no blockers and kept the 16K sentinel below the tiny16 gate: runtime default peak `7.929 GB`, `defer_to_logits` peak `8.927 GB`. XR06's per-layer comparison accepted both runtime default and `defer_to_logits`, but the XR68 policy report compared the candidate to the current runtime default and rejected it: aggregate decode moved `23556.308 -> 23587.824 ms` (`-0.134%`), `benchmark_qa_16k_001` raw/cadence p99 regressed `38.015%`, `code_review_rust_4k_001` cadence p99 regressed `9.395%`, and `code_review_rust_8k_001` cadence p95 regressed `5.909%`. Because native/cadence gates failed, XR15 MTP side-effect probes were intentionally skipped. Runtime defaults remain unchanged; `defer_to_logits` should not be the next promotion lane. |
| 2026-07-05 | XR69 native KV materialization probe | Profile complete; next lane selected | local XR69 profile ABI/docs | `native_decode_runtime_default_deferred_kv_split_profile` | `benchmarks/out/XR69-native-kv-materialization-probe/baseline-deferred-kv-split/{records.jsonl,summary.json,report.md,blockers.md,decision.md,profile.json,profile.md}` plus `codex/goals/XR69-native-kv-materialization-probe.goal.md` | Added env-gated deferred KV eval split fields to the native decode profile ABI v7 and XR06 profile artifacts: full-attention/sliding eval ms, array counts, byte counts, and decode sequence length. Non-profile runtime behavior stays on the prior single grouped `mlx::core::eval`; profiling mode evaluates the two groups separately for attribution. Static gates passed: `cargo fmt --all --check`, `git diff --check`, `cargo test -p gemma4d-ffi --lib`, and `GEMMA4D_REQUIRE_MLX=1 cargo test -p gemma4d-bench --example xr06_native_decode_tail_latency_ab --no-run`. The escalated Metal baseline run wrote `15/15` passed records and `945/945` profiled samples with no blockers; it returns `needs_more_data` only because there is no candidate variant. Peak MLX stayed under the tiny16 gate with `GEMMA4D_NATIVE_PREFILL_CHUNK_POLICY=long_context_256`: `7.321 GB` on 1K rows, `7.639 GB` at 4K, `7.718 GB` at 8K, and `7.929 GB` at 16K. Deferred KV eval is dominated by full-attention materialization: `chat_short_1k_001` `68.274 ms` total / `68.256 ms` full / `0.009 ms` sliding; `tool_json_1k_001` `63.136/63.118/0.009`; `code_review_rust_4k_001` `64.165/64.152/0.006`; `code_review_rust_8k_001` `78.003/77.984/0.009`; `benchmark_qa_16k_001` `71.018/71.001/0.009`. `attention_kv_mutation_ms` remains only `0.254..0.272 ms` mean. The next implementation should target full-attention active-KV materialization/update semantics first; sliding-window-only storage and another MTP policy sweep are lower-value. |
| 2026-07-05 | XR70 full-attention KV update candidate | Accept candidate; default-off only | local XR70 native candidate/harness/docs | `native_full_attention_kv_update_candidate_and_mtp_revisit` | `benchmarks/out/XR70-full-attention-kv-update/{full-matrix,xr66-mtp-candidate-full-attention-kv-update,xr66-mtp-sequential-oracle-full-attention-kv-update,xr70-mtp-after-native-kv-update-summary.md,xr70-mtp-after-native-kv-update-summary.json}` plus `codex/goals/XR70-full-attention-kv-update.goal.md` | Added default-off `GEMMA4D_EXPERIMENTAL_NATIVE_FULL_ATTENTION_KV_UPDATE=1`, which uses slice-update-backed full-attention active KV storage while keeping sliding-window behavior, public APIs, server defaults, MTP defaults, and the runtime default unchanged. XR06 full matrix used `GEMMA4D_NATIVE_PREFILL_CHUNK_POLICY=long_context_256` and wrote `45/45` passed records with `2835/2835` profiled samples. Candidate total decode improved versus runtime default `78020.982 -> 73634.458 ms` (`+5.622%`), summed raw p50 `363.171 -> 355.840 ms`, and summed raw p95 `403.644 -> 390.384 ms`; peak MLX stayed below the tiny16 gate, with 16K candidate peak `7.929 GB`. Full-attention deferred eval improved on 4/5 rows: `benchmark_qa_16k_001` `80.603 -> 74.261 ms`, `code_review_rust_4k_001` `66.838 -> 64.531`, `code_review_rust_8k_001` `74.884 -> 64.837`, `tool_json_1k_001` `77.571 -> 69.853`; `chat_short_1k_001` regressed `73.665 -> 76.753`. XR06 accepted candidate comparisons on `code_review_rust_8k_001` and `tool_json_1k_001`; other rows did not meet tail gates or regressed. The required XR66-compatible MTP rerun stayed `keep_experimental`: candidate exactness `12/12`, measured exactness `9/9`, oracle differential `9/9`, weighted acceptance `144/204 = 0.706`, peak MLX `8.019 GB`, protected aggregate `7474.609 -> 5991.287 ms` (`+19.845%`) below the `25%` default-on gate, selected chat/tool lanes `4835.648 -> 3347.089 ms` (`+30.784%`). MTP remains default-off. |
| 2026-07-05 | XR71 full-attention KV update capacity profile | Keep experimental; no MTP rerun | local XR71 capacity/profile ABI/docs | `native_full_attention_kv_update_capacity_v2` | `benchmarks/out/XR71-full-attention-kv-update-v2/{smoke-chat-1k-capacity,chat-1k-capacity-followup,full-matrix-256}` plus `codex/goals/XR71-full-attention-kv-update-v2.goal.md` | Added default-off `GEMMA4D_EXPERIMENTAL_NATIVE_FULL_ATTENTION_KV_UPDATE_CAPACITY={exact,64,128,256,512}` and widened the env-gated decode profile with full-attention update/capacity/slice/visible-slice timings plus capacity growth/token counters. Runtime default, public APIs, server behavior, MTP defaults, and adapters remain unchanged. Static gates passed: `cargo fmt --all --check`, `git diff --check`, `cargo test -p gemma4d-ffi --lib`, and `GEMMA4D_REQUIRE_MLX=1 cargo test -p gemma4d-bench --example xr06_native_decode_tail_latency_ab --no-run`. The low-N capacity smoke passed correctness for all variants but rejected only for fewer than three trials. The chat follow-up picked `256` as the only capacity worth the full matrix. The final XR06 matrix wrote `45/45` passed/correct records and `2835/2835` profiled decode samples. Candidate total decode improved `73947.065 -> 68583.341 ms` (`+7.253%`) and the 16K candidate peak stayed under the tiny16 gate at `7.929 GB`. The profile shows update overhead is tiny versus deferred eval: candidate update mean about `0.010 ms/token`, capacity `0.00023..0.00042 ms`, slice `0.00439..0.00460 ms`, visible-slice `0.00411..0.00432 ms`. Deferred full-attention eval means improved on all five rows, but raw tail gates stayed uneven: `chat_short_1k_001` p95/p99 regressed by `5.168%`/`26.805%`, and only `code_review_rust_8k_001` cleared the candidate XR06 tail gate. Because XR71 was not cleaner than XR70 and did not clear at least three of five tail gates, XR66 MTP side-effect was skipped. Next native focus should be full-attention deferred-eval tail jitter/synchronization, not capacity growth or broad MTP promotion. |
| 2026-07-05 | XR72 full-attention deferred-eval tail jitter | Accept candidate; profile complete | local XR72 profile ABI/docs | `native_decode_full_attention_group_profile` | `benchmarks/out/XR72-full-attention-deferred-eval-tail-jitter/{smoke-chat-1k,full-matrix}` plus `codex/goals/XR72-full-attention-deferred-eval-tail-jitter.goal.md` | Added profile-only `GEMMA4D_NATIVE_DECODE_FULL_ATTENTION_PROFILE=1` and widened the env-gated decode profile ABI to version `8` with deferred collection timing plus eight full-attention group/layer eval, array, and byte buckets. Runtime/server/MTP/default behavior stays unchanged when the flag is disabled; profile mode evaluates full-attention groups separately for attribution. Static gates passed: `cargo fmt --all --check`, `git diff --check`, `cargo test -p gemma4d-ffi --lib`, and `GEMMA4D_REQUIRE_MLX=1 cargo test -p gemma4d-bench --example xr06_native_decode_tail_latency_ab --no-run`. The full XR06 matrix wrote `45/45` passed/correct records and `2835/2835` profiled decode samples with no blockers. Runtime default was accepted against explicit per-layer on all five rows: p95/p99 improvements were `15.654/31.353%` at 16K, `11.230/28.917%` on chat, `9.638/18.941%` at 4K, `17.626/91.951%` at 8K, and `7.309/36.844%` on tool JSON. Peak MLX stayed below tiny16 at `7.321..7.929 GB`. Collection time was not material (`0.006..0.009 ms` mean on runtime default). Chat first-token outliers were dominated by full-attention group eval: runtime default first-token host latencies were `406.584..511.937 ms`, full-attention eval was `397.454..503.863 ms`, collection was `0.010..0.308 ms`, and final eval sync was about `6.337..7.000 ms`; the largest groups were early/mid full-attention layers such as 5, 11, 17, 23, and 29. The `native_decode_full_attention_kv_update_256` candidate did not beat runtime default and remains default-off. Next high-value work is XR73 scoped MTP opt-in, then XR74 native default-readiness. |
| 2026-07-05 | XR73 scoped MTP chat/tool opt-in | Accept candidate; scoped only | local XR73 harness/report/docs | `scoped_adaptive_mtp_chat_tool_opt_in` | `benchmarks/out/XR73-scoped-mtp-chat-tool-opt-in/{candidate-scoped-chat-tool,sequential-oracle-scoped-chat-tool,default-overhead-mtp-disabled,holdout-bypass-4k,xr73-scoped-mtp-summary.md,xr73-scoped-mtp-summary.json}` plus `codex/goals/XR73-scoped-mtp-chat-tool-opt-in.goal.md` | Added XR15 `--disable-mtp` default-overhead probe and `scripts/xr73_scoped_mtp_report.py`. No runtime/server default changed; MTP remains explicit/scoped/default-off. Static gates passed: `cargo fmt --all --check`, `git diff --check`, `python3 -B -m py_compile scripts/xr73_scoped_mtp_report.py`, and `GEMMA4D_REQUIRE_MLX=1 cargo test -p gemma4d-bench --example xr15_mtp_policy_variance_ab --no-run`. Candidate scoped run wrote `12/12` exact records with no blockers, selected `chat_short_1k_001:adaptive` and `tool_json_1k_001:adaptive`, protected `mtp_candidate_1k_001` by baseline bypass, and produced protected aggregate `7523.808 -> 6076.627 ms` (`+19.235%`), weighted acceptance `144/204 = 0.706`, peak MLX `8.008 GB`. Selected chat/tool lanes alone improved `+28.820%`: chat `+30.203%` with `69/96`, tool `+27.499%` with `75/108`. Sequential oracle compared `9` measured records with no missing, extra, or mismatched generated-token rows. Default-disabled overhead probe wrote `12/12` exact records and `9` measured rows with `--disable-mtp`, zero attempted drafts, zero drafter/draft/verify/repair work, no MTP events, and `-0.000%` overhead. 4K holdout probe wrote `12/12` exact records and intentionally selected no MTP workloads for `benchmark_qa_4k_001`, `code_review_rust_4k_001`, and `mtp_candidate_4k_001`; all were baseline-bypassed with zero attempted drafts. XR73 accepts the scoped opt-in surface but rejects broad default-on because protected speed remains below the `25%` gate. Next high-value work is XR74 native default-readiness. |
| 2026-07-05 | XR74 native default-readiness sweep | Ready for local persistent-native default surface | local XR74 health/TUI visibility/docs | `native_default_readiness_current_surface` | XR62 sentinel artifacts `benchmarks/out/XR61-adaptive-n-mtp/server-default-sentinel-{8k,16k,24k-low-n}` plus `codex/goals/XR74-native-default-readiness-sweep.goal.md` and `docs/evidence/XR74-native-default-readiness.md` | Added backend/native-prefill policy visibility to `/health` and the TUI dashboard without changing generation, default backend selection, native prefill policy application, MTP, or experimental native candidates. Fresh XR74 gates passed: `cargo fmt --all --check`, `git diff --check`, `cargo test -p gemma4d-server --lib`, `cargo test -p gemma4d-tui --all-targets`, and `GEMMA4D_REQUIRE_MLX=1 cargo test -p gemma4d-bench --example xr06_native_decode_tail_latency_ab --no-run`. Existing XR62 sentinels remain the tiny16 server-default evidence because XR74 did not alter the server generation path: 8K `3/3` token identity, peak `7.402 -> 7.402 GB`, policy `applied -> applied`; 16K `3/3`, peak `7.639 -> 7.639 GB`, policy `applied -> applied`; 24K low-N `2/2`, peak `7.859 -> 7.859 GB`, policy `applied -> applied`. Rollback remains explicit through `--backend stub`, `--backend real-helper`, `--backend persistent-native`, `GEMMA4D_USE_NATIVE_GRAPH=0`, and explicit native prefill env overrides. The readiness decision is scoped to local persistent-native defaults only; production internet-facing serving, broad MTP default-on, XR70/XR71 promotion, multimodal, and DSpark remain out of scope/default-off. Next high-value work is native full-attention group-eval follow-up. |
| 2026-07-05 | XR75 native full-attention group-eval follow-up | Reject candidate; keep default unchanged | local XR75 native candidate/harness/docs | `native_decode_full_attention_group_eval` | `benchmarks/out/XR75-native-full-attention-group-eval-follow-up/{smoke-chat-1k,chat-1k-followup}` plus `codex/goals/XR75-native-full-attention-group-eval-follow-up.goal.md` | Added default-off `GEMMA4D_EXPERIMENTAL_NATIVE_FULL_ATTENTION_GROUP_EVAL=1`, which serializes deferred full-attention KV eval by stable full-attention layer group while keeping the current runtime default unchanged when unset. XR06 captures/inherits the env key and exposes `native_decode_full_attention_group_eval`; no C/Rust ABI change was needed. Static gates passed during the slice: `cargo fmt --all --check`, `git diff --check`, `cargo test -p gemma4d-ffi --lib`, and `GEMMA4D_REQUIRE_MLX=1 cargo test -p gemma4d-bench --example xr06_native_decode_tail_latency_ab --no-run`. The first sandboxed smoke failed before measurement with `No Metal device available`; escalated Metal runs wrote no blockers. Smoke `smoke-chat-1k` wrote `3/3` correct records and showed low-N first-token improvement, but was rejected for fewer than three trials. The decisive `chat-1k-followup` wrote `9/9` correct records and `567/567` profiled samples with peak `7.321 GB`; runtime default remains accepted vs per-layer, but the group-eval candidate failed against runtime default: p50 regressed `69.655 -> 71.448 ms` (`+2.574%`), p95 regressed `70.191 -> 72.057 ms` (`-2.659%` improvement), and p99 improved only `163.705 -> 162.719 ms` (`+0.602%`). No full matrix was warranted. Next native focus should be non-profile first-token/full-attention tail isolation and warm/JIT/cache behavior, not serial group scheduling; MTP and DSpark remain parked. |
| 2026-07-05 | XR76 native non-profile first-token tail isolation | Accept warmup hypothesis; needs real candidate | local XR76 harness/report docs | `native_decode_runtime_default_profiled_and_warmup_probe` | `benchmarks/out/XR76-native-non-profile-first-token-tail-isolation/{profile-perturbation-chat-1k,warmup-probe-chat-1k}` plus `codex/goals/XR76-native-non-profile-first-token-tail-isolation.goal.md` | Added XR06 first-token latency aggregates from existing token traces, `native_decode_runtime_default_profiled` for variant-local `GEMMA4D_NATIVE_DECODE_PROFILE=1`, and harness-only `native_decode_runtime_default_warmup_probe`, which performs a discarded same-workload prefill plus one decode before measuring a fresh record. Runtime defaults, server behavior, public APIs, MTP defaults, adapters, and the C/Rust ABI remain unchanged. Static gates passed: `cargo fmt --all --check`, `git diff --check`, and `GEMMA4D_REQUIRE_MLX=1 cargo test -p gemma4d-bench --example xr06_native_decode_tail_latency_ab --no-run`. Profile-perturbation run wrote `9/9` correct records, `189/567` profile samples, no blockers, peak `7.321 GB`; profiled runtime default did not explain the tail, with first-token p50 `216.004 -> 218.580 ms` and p95/max `222.905 -> 229.669 ms` versus non-profile runtime default. Warmup-probe run wrote `9/9` correct records, `0/567` profile samples, no blockers, peak `7.321 GB`; the harness-only warmup candidate passed the XR06 tail gate against runtime default with raw p50 `69.778 -> 70.460 ms` (`+0.977%` regression), raw p95 `70.367 -> 71.690 ms`, raw p99/max `177.571 -> 86.680 ms` (`+51.186%`), and first-token p50 `177.571 -> 86.680 ms`, first-token p95/max `178.177 -> 97.508 ms`. This is hypothesis evidence only because warmup cost is excluded from the measured record; next native focus should be a real default-off warmup policy candidate with explicit cost/shape guardrails. Broad MTP and DSpark remain parked. |
| 2026-07-05 | XR79 warmup-aware MTP protected aggregate | Accept candidate; scoped only; broad default rejected | local XR79 report/docs | `warmup_aware_mtp_protected_aggregate` | `benchmarks/out/XR79-warmup-aware-mtp-protected-aggregate/{candidate-scoped-chat-tool,sequential-oracle-scoped-chat-tool,default-overhead-mtp-disabled,holdout-bypass-4k,xr79-warmup-aware-mtp-summary.md,xr79-warmup-aware-mtp-summary.json}` plus `codex/goals/XR79-warmup-aware-mtp-protected-aggregate.goal.md` | Extended `scripts/xr73_scoped_mtp_report.py` with backwards-compatible `--title`, `--goal`, and optional XR78 native warmup context. Runtime/server defaults, MTP defaults, native warmup policy, and DSpark behavior were unchanged. Static gates passed: `cargo fmt --all --check`, `git diff --check`, `python3 -m py_compile scripts/xr73_scoped_mtp_report.py`, an XR73 report regression invocation, and `GEMMA4D_REQUIRE_MLX=1 cargo test -p gemma4d-bench --example xr15_mtp_policy_variance_ab --no-run`. XR79 candidate wrote `12/12` exact records with no blockers, selected `chat_short_1k_001:adaptive` and `tool_json_1k_001:adaptive`, protected `mtp_candidate_1k_001` by baseline bypass, and produced protected aggregate `7479.958 -> 6022.716 ms` (`+19.482%`), weighted acceptance `144/204 = 0.706`, peak MLX `8.008 GB`. Selected chat/tool lanes alone improved `+29.237%`: chat `+30.447%` with `69/96`, tool `+28.031%` with `75/108`. Sequential oracle compared `9` measured records with no missing, extra, or generated-token mismatches. Default-disabled overhead probe wrote `12/12` exact records and `9` measured rows with zero speculative side effects and `-0.000%` overhead. 4K holdout probe wrote `12/12` exact records and intentionally selected no MTP workloads for `benchmark_qa_4k_001`, `code_review_rust_4k_001`, and `mtp_candidate_4k_001`; all were baseline-bypassed with zero attempted drafts. XR79 preserves scoped MTP value but broad default-on remains unsupported because `protected_speed_ge_gate=false`; next work should either productize the explicit scoped chat/tool opt-in or directly reduce MTP protected aggregate overhead. DSpark remains parked. |
...
python3 scripts/xr67_deferred_kv_eval_report.py --summary benchmarks/out/XR67-native-decode-deferred-kv-eval-barrier/decode-policy-ab/summary.json --out-dir benchmarks/out/XR67-native-decode-deferred-kv-eval-barrier --exclude-workload benchmark_qa_16k_001 --candidate-variant native_decode_eval_defer_to_logits --candidate-variant native_decode_eval_selective_full_attention --candidate-variant native_decode_eval_per_layer --mtp-summary runtime_default=benchmarks/out/XR67-native-decode-deferred-kv-eval-barrier/mtp-side-effect-runtime-default/summary.json --mtp-summary defer_to_logits=benchmarks/out/XR67-native-decode-deferred-kv-eval-barrier/mtp-side-effect-defer-to-logits/summary.json
GEMMA4D_REQUIRE_MLX=1 GEMMA4D_USE_NATIVE_GRAPH=1 GEMMA4D_EXPERIMENTAL_MTP_BLOCK_PREFIX_ROLLBACK=1 GEMMA4D_EXPERIMENTAL_MTP_LAZY_SECOND_DRAFT=1 GEMMA4D_EXPERIMENTAL_MTP_REAL_MARGINS=1 GEMMA4D_EXPERIMENTAL_MTP_ADAPTIVE_N=1 cargo run -p gemma4d-bench --example xr15_mtp_policy_variance_ab -- --out-dir benchmarks/out/XR67-native-decode-deferred-kv-eval-barrier/mtp-side-effect-runtime-default --source-replay benchmarks/out/XR56-repair-cost/candidate-retro-prefix/summary.json --trials 3 --warmups 1 --max-new-tokens 64 --block-sizes 1,2,3,4,6,8 --adaptive-policy xr61-real-margin-v1 --clear-workload-ids --workload-id chat_short_1k_001 --workload-id tool_json_1k_001 --workload-id mtp_candidate_1k_001
GEMMA4D_REQUIRE_MLX=1 GEMMA4D_USE_NATIVE_GRAPH=1 GEMMA4D_NATIVE_DECODE_KV_EVAL=defer_to_logits GEMMA4D_EXPERIMENTAL_MTP_BLOCK_PREFIX_ROLLBACK=1 GEMMA4D_EXPERIMENTAL_MTP_LAZY_SECOND_DRAFT=1 GEMMA4D_EXPERIMENTAL_MTP_REAL_MARGINS=1 GEMMA4D_EXPERIMENTAL_MTP_ADAPTIVE_N=1 cargo run -p gemma4d-bench --example xr15_mtp_policy_variance_ab -- --out-dir benchmarks/out/XR67-native-decode-deferred-kv-eval-barrier/mtp-side-effect-defer-to-logits --source-replay benchmarks/out/XR56-repair-cost/candidate-retro-prefix/summary.json --trials 3 --warmups 1 --max-new-tokens 64 --block-sizes 1,2,3,4,6,8 --adaptive-policy xr61-real-margin-v1 --clear-workload-ids --workload-id chat_short_1k_001 --workload-id tool_json_1k_001 --workload-id mtp_candidate_1k_001
```
...
cargo test -p gemma4d-ffi --lib
GEMMA4D_REQUIRE_MLX=1 cargo test -p gemma4d-bench --example xr06_native_decode_tail_latency_ab --no-run
GEMMA4D_REQUIRE_MLX=1 GEMMA4D_USE_NATIVE_GRAPH=1 GEMMA4D_NATIVE_DECODE_PROFILE=1 GEMMA4D_NATIVE_DECODE_FULL_ATTENTION_PROFILE=1 GEMMA4D_NATIVE_PREFILL_CHUNK_POLICY=long_context_256 cargo run -p gemma4d-bench --example xr06_native_decode_tail_latency_ab -- --out-dir benchmarks/out/XR72-full-attention-deferred-eval-tail-jitter/smoke-chat-1k --trials 1 --max-new-tokens 16 --clear-workload-ids --workload-id chat_short_1k_001 --variants native_decode_eval_per_layer,native_decode_runtime_default,native_decode_full_attention_kv_update_256
GEMMA4D_REQUIRE_MLX=1 GEMMA4D_USE_NATIVE_GRAPH=1 GEMMA4D_NATIVE_DECODE_PROFILE=1 GEMMA4D_NATIVE_DECODE_FULL_ATTENTION_PROFILE=1 GEMMA4D_NATIVE_PREFILL_CHUNK_POLICY=long_context_256 cargo run -p gemma4d-bench --example xr06_native_decode_tail_latency_ab -- --out-dir benchmarks/out/XR72-full-attention-deferred-eval-tail-jitter/full-matrix --trials 3 --max-new-tokens 64 --clear-workload-ids --workload-id chat_short_1k_001 --workload-id tool_json_1k_001 --workload-id code_review_rust_4k_001 --workload-id code_review_rust_8k_001 --workload-id benchmark_qa_16k_001 --variants native_decode_eval_per_layer,native_decode_runtime_default,native_decode_full_attention_kv_update_256
```
...
cargo fmt --all --check
git diff --check
python3 -B -m py_compile scripts/xr73_scoped_mtp_report.py
GEMMA4D_REQUIRE_MLX=1 cargo test -p gemma4d-bench --example xr15_mtp_policy_variance_ab --no-run
GEMMA4D_REQUIRE_MLX=1 GEMMA4D_USE_NATIVE_GRAPH=1 GEMMA4D_EXPERIMENTAL_MTP_BLOCK_PREFIX_ROLLBACK=1 GEMMA4D_EXPERIMENTAL_MTP_LAZY_SECOND_DRAFT=1 GEMMA4D_EXPERIMENTAL_MTP_REAL_MARGINS=1 GEMMA4D_EXPERIMENTAL_MTP_ADAPTIVE_N=1 cargo run -p gemma4d-bench --example xr15_mtp_policy_variance_ab -- --out-dir benchmarks/out/XR73-scoped-mtp-chat-tool-opt-in/candidate-scoped-chat-tool --source-replay benchmarks/out/XR56-repair-cost/candidate-retro-prefix/summary.json --trials 3 --warmups 1 --max-new-tokens 32 --block-sizes 1,2,3,4,6,8 --adaptive-policy xr61-real-margin-v1 --adaptive-zero-accept-run 3 --adaptive-min-generated-tokens 12 --clear-workload-ids --workload-id chat_short_1k_001 --workload-id tool_json_1k_001 --workload-id mtp_candidate_1k_001
GEMMA4D_REQUIRE_MLX=1 GEMMA4D_USE_NATIVE_GRAPH=1 GEMMA4D_EXPERIMENTAL_MTP_LAZY_SECOND_DRAFT=1 GEMMA4D_EXPERIMENTAL_MTP_REAL_MARGINS=1 GEMMA4D_EXPERIMENTAL_MTP_ADAPTIVE_N=1 cargo run -p gemma4d-bench --example xr15_mtp_policy_variance_ab -- --out-dir benchmarks/out/XR73-scoped-mtp-chat-tool-opt-in/sequential-oracle-scoped-chat-tool --source-replay benchmarks/out/XR56-repair-cost/candidate-retro-prefix/summary.json --trials 3 --warmups 1 --max-new-tokens 32 --block-sizes 1,2,3,4,6,8 --adaptive-policy xr61-real-margin-v1 --adaptive-zero-accept-run 3 --adaptive-min-generated-tokens 12 --clear-workload-ids --workload-id chat_short_1k_001 --workload-id tool_json_1k_001 --workload-id mtp_candidate_1k_001
GEMMA4D_REQUIRE_MLX=1 GEMMA4D_USE_NATIVE_GRAPH=1 cargo run -p gemma4d-bench --example xr15_mtp_policy_variance_ab -- --out-dir benchmarks/out/XR73-scoped-mtp-chat-tool-opt-in/default-overhead-mtp-disabled --source-replay benchmarks/out/XR56-repair-cost/candidate-retro-prefix/summary.json --trials 3 --warmups 1 --max-new-tokens 32 --block-sizes 1,2,3,4,6,8 --disable-mtp --clear-workload-ids --workload-id chat_short_1k_001 --workload-id tool_json_1k_001 --workload-id mtp_candidate_1k_001
GEMMA4D_REQUIRE_MLX=1 GEMMA4D_USE_NATIVE_GRAPH=1 GEMMA4D_EXPERIMENTAL_MTP_BLOCK_PREFIX_ROLLBACK=1 GEMMA4D_EXPERIMENTAL_MTP_LAZY_SECOND_DRAFT=1 GEMMA4D_EXPERIMENTAL_MTP_REAL_MARGINS=1 GEMMA4D_EXPERIMENTAL_MTP_ADAPTIVE_N=1 cargo run -p gemma4d-bench --example xr15_mtp_policy_variance_ab -- --out-dir benchmarks/out/XR73-scoped-mtp-chat-tool-opt-in/holdout-bypass-4k --source-replay benchmarks/out/XR56-repair-cost/candidate-retro-prefix/summary.json --trials 3 --warmups 1 --max-new-tokens 32 --block-sizes 1,2,3,4,6,8 --adaptive-policy xr61-real-margin-v1 --adaptive-zero-accept-run 3 --adaptive-min-generated-tokens 12 --clear-workload-ids --workload-id benchmark_qa_4k_001 --workload-id code_review_rust_4k_001 --workload-id mtp_candidate_4k_001
python3 scripts/xr73_scoped_mtp_report.py --candidate-summary benchmarks/out/XR73-scoped-mtp-chat-tool-opt-in/candidate-scoped-chat-tool/summary.json --oracle-summary benchmarks/out/XR73-scoped-mtp-chat-tool-opt-in/sequential-oracle-scoped-chat-tool/summary.json --default-overhead-summary benchmarks/out/XR73-scoped-mtp-chat-tool-opt-in/default-overhead-mtp-disabled/summary.json --holdout-summary benchmarks/out/XR73-scoped-mtp-chat-tool-opt-in/holdout-bypass-4k/summary.json --out-dir benchmarks/out/XR73-scoped-mtp-chat-tool-opt-in
```
...
python3 scripts/xr73_scoped_mtp_report.py --candidate-summary benchmarks/out/XR73-scoped-mtp-chat-tool-opt-in/candidate-scoped-chat-tool/summary.json --oracle-summary benchmarks/out/XR73-scoped-mtp-chat-tool-opt-in/sequential-oracle-scoped-chat-tool/summary.json --default-overhead-summary benchmarks/out/XR73-scoped-mtp-chat-tool-opt-in/default-overhead-mtp-disabled/summary.json --holdout-summary benchmarks/out/XR73-scoped-mtp-chat-tool-opt-in/holdout-bypass-4k/summary.json --out-dir /private/tmp/xr73-report-regression
GEMMA4D_REQUIRE_MLX=1 cargo test -p gemma4d-bench --example xr15_mtp_policy_variance_ab --no-run
GEMMA4D_REQUIRE_MLX=1 GEMMA4D_USE_NATIVE_GRAPH=1 GEMMA4D_EXPERIMENTAL_MTP_BLOCK_PREFIX_ROLLBACK=1 GEMMA4D_EXPERIMENTAL_MTP_LAZY_SECOND_DRAFT=1 GEMMA4D_EXPERIMENTAL_MTP_REAL_MARGINS=1 GEMMA4D_EXPERIMENTAL_MTP_ADAPTIVE_N=1 cargo run -p gemma4d-bench --example xr15_mtp_policy_variance_ab -- --out-dir benchmarks/out/XR79-warmup-aware-mtp-protected-aggregate/candidate-scoped-chat-tool --source-replay benchmarks/out/XR56-repair-cost/candidate-retro-prefix/summary.json --trials 3 --warmups 1 --max-new-tokens 32 --block-sizes 1,2,3,4,6,8 --adaptive-policy xr61-real-margin-v1 --adaptive-zero-accept-run 3 --adaptive-min-generated-tokens 12 --clear-workload-ids --workload-id chat_short_1k_001 --workload-id tool_json_1k_001 --workload-id mtp_candidate_1k_001
GEMMA4D_REQUIRE_MLX=1 GEMMA4D_USE_NATIVE_GRAPH=1 GEMMA4D_EXPERIMENTAL_MTP_LAZY_SECOND_DRAFT=1 GEMMA4D_EXPERIMENTAL_MTP_REAL_MARGINS=1 GEMMA4D_EXPERIMENTAL_MTP_ADAPTIVE_N=1 cargo run -p gemma4d-bench --example xr15_mtp_policy_variance_ab -- --out-dir benchmarks/out/XR79-warmup-aware-mtp-protected-aggregate/sequential-oracle-scoped-chat-tool --source-replay benchmarks/out/XR56-repair-cost/candidate-retro-prefix/summary.json --trials 3 --warmups 1 --max-new-tokens 32 --block-sizes 1,2,3,4,6,8 --adaptive-policy xr61-real-margin-v1 --adaptive-zero-accept-run 3 --adaptive-min-generated-tokens 12 --clear-workload-ids --workload-id chat_short_1k_001 --workload-id tool_json_1k_001 --workload-id mtp_candidate_1k_001
GEMMA4D_REQUIRE_MLX=1 GEMMA4D_USE_NATIVE_GRAPH=1 cargo run -p gemma4d-bench --example xr15_mtp_policy_variance_ab -- --out-dir benchmarks/out/XR79-warmup-aware-mtp-protected-aggregate/default-overhead-mtp-disabled --source-replay benchmarks/out/XR56-repair-cost/candidate-retro-prefix/summary.json --trials 3 --warmups 1 --max-new-tokens 32 --block-sizes 1,2,3,4,6,8 --disable-mtp --clear-workload-ids --workload-id chat_short_1k_001 --workload-id tool_json_1k_001 --workload-id mtp_candidate_1k_001
GEMMA4D_REQUIRE_MLX=1 GEMMA4D_USE_NATIVE_GRAPH=1 GEMMA4D_EXPERIMENTAL_MTP_BLOCK_PREFIX_ROLLBACK=1 GEMMA4D_EXPERIMENTAL_MTP_LAZY_SECOND_DRAFT=1 GEMMA4D_EXPERIMENTAL_MTP_REAL_MARGINS=1 GEMMA4D_EXPERIMENTAL_MTP_ADAPTIVE_N=1 cargo run -p gemma4d-bench --example xr15_mtp_policy_variance_ab -- --out-dir benchmarks/out/XR79-warmup-aware-mtp-protected-aggregate/holdout-bypass-4k --source-replay benchmarks/out/XR56-repair-cost/candidate-retro-prefix/summary.json --trials 3 --warmups 1 --max-new-tokens 32 --block-sizes 1,2,3,4,6,8 --adaptive-policy xr61-real-margin-v1 --adaptive-zero-accept-run 3 --adaptive-min-generated-tokens 12 --clear-workload-ids --workload-id benchmark_qa_4k_001 --workload-id code_review_rust_4k_001 --workload-id mtp_candidate_4k_001
python3 scripts/xr73_scoped_mtp_report.py --title "XR79 Warmup-Aware MTP Protected Aggregate" --goal XR79-warmup-aware-mtp-protected-aggregate --candidate-summary benchmarks/out/XR79-warmup-aware-mtp-protected-aggregate/candidate-scoped-chat-tool/summary.json --oracle-summary benchmarks/out/XR79-warmup-aware-mtp-protected-aggregate/sequential-oracle-scoped-chat-tool/summary.json --default-overhead-summary benchmarks/out/XR79-warmup-aware-mtp-protected-aggregate/default-overhead-mtp-disabled/summary.json --holdout-summary benchmarks/out/XR79-warmup-aware-mtp-protected-aggregate/holdout-bypass-4k/summary.json --native-warmup-summary benchmarks/out/XR78-native-amortized-warmup-matrix/amortized-1k-4k-trials3/summary.json --native-warmup-report benchmarks/out/XR78-native-amortized-warmup-matrix/amortized-1k-4k-trials3/report.md --out-dir benchmarks/out/XR79-warmup-aware-mtp-protected-aggregate --out-md xr79-warmup-aware-mtp-summary.md --out-json xr79-warmup-aware-mtp-summary.json
```
...
cargo test -p gemma4d-ffi --lib
GEMMA4D_REQUIRE_MLX=1 cargo test -p gemma4d-bench --example xr06_native_decode_tail_latency_ab --no-run
GEMMA4D_REQUIRE_MLX=1 GEMMA4D_USE_NATIVE_GRAPH=1 GEMMA4D_NATIVE_DECODE_PROFILE=1 GEMMA4D_NATIVE_PREFILL_CHUNK_POLICY=long_context_256 cargo run -p gemma4d-bench --example xr06_native_decode_tail_latency_ab -- --out-dir benchmarks/out/XR75-native-full-attention-group-eval-follow-up/smoke-chat-1k --trials 1 --max-new-tokens 16 --clear-workload-ids --workload-id chat_short_1k_001 --variants native_decode_eval_per_layer,native_decode_runtime_default,native_decode_full_attention_group_eval
GEMMA4D_REQUIRE_MLX=1 GEMMA4D_USE_NATIVE_GRAPH=1 GEMMA4D_NATIVE_DECODE_PROFILE=1 GEMMA4D_NATIVE_PREFILL_CHUNK_POLICY=long_context_256 cargo run -p gemma4d-bench --example xr06_native_decode_tail_latency_ab -- --out-dir benchmarks/out/XR75-native-full-attention-group-eval-follow-up/chat-1k-followup --trials 3 --max-new-tokens 64 --clear-workload-ids --workload-id chat_short_1k_001 --variants native_decode_eval_per_layer,native_decode_runtime_default,native_decode_full_attention_group_eval
```
...
git diff --check
GEMMA4D_REQUIRE_MLX=1 cargo test -p gemma4d-bench --example xr06_native_decode_tail_latency_ab --no-run
GEMMA4D_REQUIRE_MLX=1 GEMMA4D_USE_NATIVE_GRAPH=1 GEMMA4D_NATIVE_PREFILL_CHUNK_POLICY=long_context_256 cargo run -p gemma4d-bench --example xr06_native_decode_tail_latency_ab -- --out-dir benchmarks/out/XR76-native-non-profile-first-token-tail-isolation/profile-perturbation-chat-1k --trials 3 --max-new-tokens 64 --clear-workload-ids --workload-id chat_short_1k_001 --variants native_decode_eval_per_layer,native_decode_runtime_default,native_decode_runtime_default_profiled
GEMMA4D_REQUIRE_MLX=1 GEMMA4D_USE_NATIVE_GRAPH=1 GEMMA4D_NATIVE_PREFILL_CHUNK_POLICY=long_context_256 cargo run -p gemma4d-bench --example xr06_native_decode_tail_latency_ab -- --out-dir benchmarks/out/XR76-native-non-profile-first-token-tail-isolation/warmup-probe-chat-1k --trials 3 --max-new-tokens 64 --clear-workload-ids --workload-id chat_short_1k_001 --variants native_decode_eval_per_layer,native_decode_runtime_default,native_decode_runtime_default_warmup_probe
```
...
|---|---|---|---|
| 2026-07-04 | Added XR64 env-gated native decode stage profiling. `GEMMA4D_NATIVE_DECODE_PROFILE=1` now records per-token native decode stage timings through the narrow step-result ABI, and XR06 writes optional trace fields plus `profile.json`/`profile.md`; disabled native decode behavior and defaults are unchanged. | `native/gemma4_mlx/include/gemma4_mlx.h`, `native/gemma4_mlx/src/native_model.cc`, `native/gemma4_mlx/src/runtime.cc`, `crates/gemma4d-ffi/src/lib.rs`, `crates/gemma4d-bench/examples/xr06_native_decode_tail_latency_ab.rs`, `codex/goals/XR64-native-decode-stage-profile.goal.md`, `BENCHMARKS.md` | `cargo fmt`; `cargo test -p gemma4d-ffi`; `cargo check -p gemma4d-bench --example xr06_native_decode_tail_latency_ab`; XR64 profile run with approved Metal access wrote `378/378` profiled decode samples. |
...
|---|---|---|---|
| 2026-07-04 | `cargo fmt` | Passed | Formatting gate after XR64 native decode profile ABI and XR06 artifact changes. |
| 2026-07-04 | `cargo test -p gemma4d-ffi` | Passed | `18` passed, `1` ignored; includes ABI version `5`, `Gemma4DecodeProfileInfo` layout pin, and full `Gemma4StepResult` size/offset pin. |
| 2026-07-04 | `cargo check -p gemma4d-bench --example xr06_native_decode_tail_latency_ab` | Passed | Focused compile gate for XR06 profile trace and profile report generation. |
| 2026-07-04 | `GEMMA4D_REQUIRE_MLX=1 GEMMA4D_USE_NATIVE_GRAPH=1 GEMMA4D_NATIVE_DECODE_PROFILE=1 cargo run -p gemma4d-bench --example xr06_native_decode_tail_latency_ab -- --out-dir benchmarks/out/XR61-adaptive-n-mtp/native-decode-profile --trials 3 --max-new-tokens 64 --clear-workload-ids --workload-id chat_short_1k_001 --workload-id tool_json_1k_001 --variant native_decode_eval_per_layer` | Passed | Sandboxed attempt failed before benchmarking with `No Metal device available`; escalated Metal rerun wrote `6/6` passed records, `378/378` profiled samples, `profile.json`, and `profile.md`. Decision is `needs_more_data` as an A/B run because only the baseline variant was selected; P4 interpretation is profiling complete with no native decode default change. |
...
- XR55 proves native MTP block sizes `{1,2,3,4,6,8}` can remain byte-identical
  ...
  Sequential commit semantics are frozen by the sequential-oracle contract;
  trace and ABI plumbing may change only with differential generated-token
  evidence.
- XR56 removes the dead accepted-prefix re-forward from the default-off
  block-prefix MTP path by retroactively slicing prefix KV from the first
  batched verify forward. This is a real implementation improvement, not a
  default-on decision: the guarded policy improved to `+21.989%` but stayed
  below the `25%` gate. Fixed `N=4` rose to `+17.609%`, yet the guarded policy
  selected `chat_short_1k_001:N=3` and `tool_json_1k_001:N=6`; the tool JSON
  N=6 pick is within noise of N=3/N=4 at 3 measured trials, so wave-5 should
  not overfit that exact block choice. N=6/N=8 improved but remain
  acceptance-bound, and `mtp_candidate_1k_001` remains unselected with
  `18/18` measured auto-disable records. XR56 timing fields are ABI-v3:
  `repair_forward_ms=0.0` means the dead prefix forward is gone, while
  remaining `verify_repair_ms` is fallback decode. Sequential commit semantics
  remain frozen by the sequential-oracle contract; trace and ABI plumbing may
  change only with differential generated-token evidence.
- XR57 retires the "margins are fake" boundary only for records captured with
  `GEMMA4D_EXPERIMENTAL_MTP_REAL_MARGINS=1` under ABI v4 or later. In the
  default path, XR57 intentionally records target top-1 and zero drafter score
  fields to protect decode latency; real target top-5, drafter top-1 logits,
  drafter top1-top2 margins, and `draft_in_top_k` membership are available only
  in the env-gated instrumentation leg. Pre-XR57 `draft_logits`,
  `logit_margins`, target top-k beyond rank 1, and `draft_in_top_k` fields are
  historical-fake and must not be used for adaptive-N analysis except as
  legacy placeholders. XR57 changes signal capture only; adaptive-N policy
  logic remains wave-5 work.
...
- XR06 accepts native decode eval scheduling as an opt-in experimental candidate
  only in the original XR06 evidence. XR65 supersedes the default-policy part
  with split-profile evidence and runtime-default validation.
...
- XR64 is profiling evidence only. It widens the native step-result ABI to
  carry env-gated decode-stage timings, but it does not change native decode
  behavior, server defaults, MTP policy, or decode eval scheduling defaults.
  The only material lane observed is broad `forward_graph_ms`, which includes
  native layer graph work plus decode KV append/cache mutation; FFI, greedy
  selection, scalar reads, hidden-view handoff, and peak-memory telemetry are
  sub-millisecond cleanup lanes and should not drive the next optimization.
- XR65 splits XR64's broad `forward_graph_ms` lane and shows the old explicit
  per-layer path is dominated by decode KV mutation/immediate KV eval, not
  non-KV graph work. The native runtime default now uses grouped
  end-of-decode KV eval, while explicit `per_layer`/`current` keeps the old
  behavior for comparison or rollback.
- XR69 further splits the runtime-default deferred KV eval barrier and shows the
  next high-value native decode lane is full-attention active-KV
  materialization. Sliding-window eval is effectively noise in the profile
  (`0.006..0.009 ms` mean), while full-attention deferred KV eval accounts for
  nearly all of the `63..78 ms/token` grouped barrier. A sliding-only
  fixed-capacity slab is therefore not the best next bet; the next candidate
  should be default-off and attack full-attention update/materialization
  semantics directly.
- XR70 implements that full-attention update/materialization candidate as
  default-off and proves a real but uneven win. Total XR06 decode improves
  `78020.982 -> 73634.458 ms` (`+5.622%`) and full-attention deferred eval
  improves on 4/5 rows, but `chat_short_1k_001` regresses and only
  `code_review_rust_8k_001` plus `tool_json_1k_001` clear the XR06 candidate
  tail gate. Treat this as a useful experimental kernel/default-off option,
  not a default promotion without a follow-up policy or a cleaner kernel.
- XR66 shows the faster XR65 native decode default does not close the broad MTP
  default-on case. The selected chat/tool lanes remain above `25%`, but the
  XR61-compatible protected aggregate including the baseline-bypassed
  `mtp_candidate_1k_001` is only `+20.334%`, down from XR61's `+21.303%`.
  Keep Adaptive-N MTP env-gated/default-off until a broader policy clears the
  protected aggregate, holdout, oracle, memory, default-overhead, and risk
  gates together.
- XR70's required MTP rerun keeps that conclusion intact: protected aggregate
  speedup is `+19.845%`, selected chat/tool lanes remain strong at `+30.784%`,
  exactness and oracle checks pass, and MTP still misses the broad `25%`
  default-on gate.
- XR71 narrows XR70's full-attention KV update capacity policy and adds
  profile splits for capacity growth, slice update, and visible-slice creation.
  The `256` capacity policy improves aggregate XR06 decode
  `73947.065 -> 68583.341 ms` (`+7.253%`) and keeps the 16K sentinel under the
  tiny16 gate at `7.929 GB`, but it is not clean enough for promotion:
  `chat_short_1k_001` regressed raw p95/p99 by `5.168%`/`26.805%`, and only
  `code_review_rust_8k_001` cleared the candidate tail gate. The new profile
  fields show update/capacity/slice/visible-slice overhead is about
  `0.010 ms/token`; the remaining high-value native lane is full-attention
  deferred-eval tail jitter/synchronization. XR66 MTP side-effect was skipped
  because XR71 did not meet the cleaner-than-XR70 gate.
