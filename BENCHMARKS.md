...
|---|---|---:|---:|---:|---|---|---|
| Server model-path default | `benchmark_qa_16k_001` | `88657.954` | `42268.699` | `+52.324%` | `21.874 -> 7.638 GB` | Accepted, CLI default path | XR53 `default-path-16k-repeats3` |
...
| 2026-07-03 | XR53 server default backend and admission estimator | Accept candidate | local XR53 server default changes | `server_real_helper_vs_default_persistent_native_real_contexts` | `benchmarks/out/XR53-server-default-backend/{default-path-1k-repeats3,default-path-16k-repeats3}` plus `codex/goals/XR53-server-default-backend-estimator.goal.md` | `gemma4d serve --model-path PATH` now defaults to `ServerBackend::PersistentNative` when no backend flag is explicit; zero-arg/no-model-path and explicit `--backend stub` still use the M11 stub, and explicit `--backend real-helper` remains helper-backed. Removed the `GEMMA4D_EXPERIMENTAL_PERSISTENT_SERVER=1` gate. Added `admission_prefill_chunked` to `ServerConfig`, set from the XR51 pure chunk-policy selector for PersistentNative serve configs, and replaced the old `(prompt + max_tokens) * 4096` guard with XR51/P04 constants: resident base `7,864,036,352 B`, decode KV `16,384 B/token`, chunked prefill `31 KiB/token` above 1K, unchunked interpolation across 1K/4K/8K/16K measured points, fail-closed above 16K unchunked, and `13/10` BPE correction on the current whitespace prompt estimate. Unit tests verify the estimator table and that a 16K unchunked request at `14336 MB` is rejected while chunked is admitted. Verification passed: `cargo fmt --all --check`; `git diff --check`; `cargo test -p gemma4d-server --all-targets`; `cargo test -p gemma4d-bench --example xr11_persistent_native_server_ab --no-run`. Escalated Metal A/B commands used `GEMMA4D_REQUIRE_MLX=1 GEMMA4D_USE_NATIVE_GRAPH=1 cargo run -p gemma4d-bench --example xr11_persistent_native_server_ab` with `--model-path artifacts/models/gemma-4-12B-it-4bit --workloads benchmarks/workloads/real-contexts/workloads.jsonl --repeats 3 --max-new-tokens 1 --max-context-tokens 32768 --memory-budget-mb 14336`; the candidate startup path called `parse_serve_options` with `--model-path` and no `--backend`. Both 1K and 16K runs passed with token identity `3/3`, no blockers, candidate runtime snapshots reporting `persistent_native`, and load count `3 -> 1`. `chat_short_1k_001`: prefill p50/p95 `2869.853/2984.829 -> 2309.716/2963.324 ms`, peak `7.324 -> 7.324 GB`. `benchmark_qa_16k_001`: `88657.954/89421.493 -> 42268.699/57144.218 ms`, peak `21.874 -> 7.638 GB`. |
...
- XR53 admission estimates are conservative guardrails, not exact tokenizer or
  allocator predictions. They use XR51/P04 measured constants, a `13/10` BPE
  correction over the current whitespace estimate, chunked constants only when
  the server-owned XR51 default chunk policy is known active, and unchunked
  worst-case constants otherwise. Explicit native chunk env overrides are
  treated as unknown and therefore unchunked for admission until a native policy
  getter exists.
